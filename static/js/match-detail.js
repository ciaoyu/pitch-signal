...
(function() {
    ...
    async function openMatch(id) {
        ...
        const showPreMatch = (!isFinishedMatch && (scheduledMatch.state === 'pre' || (matchData.status?.type?.name || '').includes('SCHEDULED')))
            || (isFinishedMatch && (scheduledMatch.stage === 'knockout' || matchData.hasPenalties === true));
        ...
        html += `<button data-action="switch-detail-tab" data-detail-tab="corners" role="tab" aria-selected="false" class="detail-tab px-3 py-1.5 rounded-lg text-[11px] font-bold bg-white/5 text-gray-400 transition whitespace-nowrap" style="min-width:44px;min-height:44px">📐 ${tx('角球', 'Corners')}</button>`;
        html += `<button data-action="switch-detail-tab" data-detail-tab="coach" role="tab" aria-selected="false" class="detail-tab px-3 py-1.5 rounded-lg text-[11px] font-bold bg-white/5 text-gray-400 transition whitespace-nowrap" style="min-width:44px;min-height:44px">🧠 ${tx('教练', 'Coach')}</button>`;
        html += `<button data-action="switch-detail-tab" data-detail-tab="venue-tab" role="tab" aria-selected="false" class="detail-tab px-3 py-1.5 rounded-lg text-[11px] font-bold bg-white/5 text-gray-400 transition whitespace-nowrap" style="min-width:44px;min-height:44px">🏟️ ${tx('场地详情', 'Venue')}</button>`;
        html += `</div>`;
    ...
    function renderVenueWeather(data) {
        ...
        const grassDisplay=v.grass||(meta?.surfaceI18n
        ...
        const weatherBlock=w
            ? `<div class="glass-light rounded-lg p-3"><div class="flex items-center justify-between mb-2"><span class="text-xs font-bold text-gray-400">${weatherIcon} ${tx('天气状况','Weather')}</span><span class="text-[11px] text-gray-500">${esc(w.description)||''}</span></div><div class="grid grid-cols-3 gap-3 text-center"><div><div class="text-xl font-bold">${esc(w.temp)||'-'}°C</div><div class="text-[11px] text-gray-500">${tx('温度','Temp')}</div><div class="text-[11px] text-gray-600">${tx('体感','Feels')} ${esc(w.feelsLike)||'-'}°C</div></div><div><div class="text-xl font-bold">${esc(w.humidity)||'-'}%</div><div class="text-[11px] text-gray-500">${tx('湿度','Humidity')}</div></div><div><div class="text-xl font-bold">${w.windSpeed?esc(Math.round(w.windSpeed)):'-'}</div><div class="text-[11px] text-gray-500">${tx('风速','Wind')} km/h</div></div></div></div>`
            : `<div class="glass-light rounded-lg p-3"><div class="text-center text-gray-500 text-xs"><div class="mb-1">🌤️ ${tx('天气数据','Weather')}</div><div>${tx('暂无实时天气','No weather data')}</div></div></div>`;
        const impactBlock=impact
            ? `<div class="glass-light rounded-lg p-3"><div class="text-xs font-bold text-gray-400 mb-2">📋 ${tx('场地事实','Venue Facts')}</div>${impact.details?.map(d=>`<div class="text-[11px] text-gray-300 mb-1">• ${esc(d)}</div>`).join('')||''}<div class="text-[10px] text-gray-600 mt-2">${esc(window.WorldCup.I18n.i18nText(impact.disclaimerI18n,''))}</div></div>`
            : '';
    ...
    function renderCornerAnalysis(data) {
        ...
        const trend=v.trend||'neutral',trendEmoji={over_strong:'🔴',over_slight:'🟠',under_strong:'🔵',under_slight:'🟤'}[trend]||'⚪';
        const conf=v.confidence==='high'?'🟢':v.confidence==='medium'?'🟡':'⚪';
        ...
        return `<div class="space-y-3"><div class="glass-light rounded-lg p-3"><div class="flex items-center justify-between mb-2"><span class="text-xs font-bold text-gray-400">📐 ${tx('角球预测','Corner Forecast')}</span><span class="text-xs text-gray-500">${lineLabel} <span class="font-bold text-white">${lineDisplay}</span></span></div><div class="flex items-center gap-3 mb-3"><div class="text-center"><div class="text-2xl font-bold text-white">${p.total??'-'}</div><div class="text-[11px] text-gray-500">${tx('预测总角球','Projected Corners')}</div><div class="text-[9px] text-gray-600">${tx('实验性估算','Experimental estimate')}</div></div><div class="flex-1"><div class="flex items-center gap-1 mb-1"><span class="text-xs">${trendEmoji}</span><span class="text-xs font-bold ${trend.includes('over')?'text-red-400':trend.includes('under')?'text-blue-400':'text-gray-400'}">${esc(trend.replace('_',' ').toUpperCase())}</span><span class="ml-auto">${conf}</span></div><div class="text-[11px] text-gray-500">${tx('实际','Actual')} <span class="font-bold text-white">${r.current?.total||0}</span>${hasLine?` / ${line}`:''}</div></div></div><div class="relative h-4 bg-white/5 rounded-full overflow-hidden mb-2"><div class="absolute top-0 bottom-0 w-0.5 bg-yellow-500/50" style="left:${expectedPct}%"></div><div class="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-700" style="width:${progressPct}%"></div><div class="absolute inset-0 flex items-center justify-between px-2 text-[9px]"><span class="text-white font-bold">${r.current?.total||0}</span><span class="text-gray-400">${Math.round(progressPct)}%</span></div></div><div class="flex items-center justify-between text-[11px]"><span class="text-gray-500">${tx('本届数据，向赛事均值收缩','Tournament data, mean-shrunk')}</span><span class="${paceColor} font-bold">${paceStatus}</span></div></div><div class="grid grid-cols-2 gap-2"><div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">🔵 ${tx('主队','Home')}</div><div class="text-sm font-bold">${h.homeAvg??'-'} ${tx('场均角球','corners for')}</div><div class="text-[11px] text-gray-600">${tx('场均被角球','conceded')} ${h.homeAgainstAvg??'-'} · ${h.homeSampleCount||0}${tx('场',' matches')}</div></div><div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">🔴 ${tx('客队','Away')}</div><div class="text-sm font-bold">${h.awayAvg??'-'} ${tx('场均角球','corners for')}</div><div class="text-[11px] text-gray-600">${tx('场均被角球','conceded')} ${h.awayAgainstAvg??'-'} · ${h.awaySampleCount||0}${tx('场',' matches')}</div></div></div>${v.reason||v.reasonI18n?`<div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">📊 ${tx('分析结论','Verdict')}</div><div class="text-xs text-gray-300">${esc(window.WorldCup.I18n.i18nText(v.reasonI18n,v.reason||''))}</div></div>`:''}</div>`;
    }
