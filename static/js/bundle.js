...
// data/player_name_zh.json
var player_name_zh_default;
var init_player_name_zh = __esm({
  "data/player_name_zh.json"() {
    player_name_zh_default = {
  ...
  "static/js/i18n.js"() {
    ...
    (function() {
      ...
      function getZhNamesLower() {
        if (!_zhNamesLower) {
          _zhNamesLower = {};
          for (const [k, v] of Object.entries(player_name_zh_default)) {
      ...
      function translatePlayerName(name, nameZh) {
        ...
        if (player_name_zh_default[name]) return player_name_zh_default[name];
      ...
      function translateCoachField(val, type) {
        ...
        if (state.uiLang === "zh") {
          ...
          return player_name_zh_default[val] || val;
        } else {
          if (type === "tenure") {
            return String(val).replace("\u5E74", " years").replace("\u4E2A\u6708", " months");
          }
          if (type === "style" || type === "nationality") {
            const revDict = Object.fromEntries(Object.entries(player_name_zh_default).map(([k, v]) => [v, k]));
      ...
      function displayTeamName(name) {
        ...
        if (!bilingual) {
          if (state.uiLang === "zh" && player_name_zh_default[raw]) return player_name_zh_default[raw];
          return raw;
        }
      ...
      window.WorldCup.I18n = {
        ZH_NAMES: player_name_zh_default,
        translatePlayerName,
        translateCoachField,
        t,
        displayTeamName,
  ...
  "static/js/match-detail.js"() {
    (function() {
      ...
      async function openMatch(id) {
        ...
        const showPreMatch = !isFinishedMatch && (scheduledMatch.state === "pre" || (matchData2.status?.type?.name || "").includes("SCHEDULED")) || isFinishedMatch && (scheduledMatch.stage === "knockout" || matchData2.hasPenalties === true);
        ...
        if (isLive || isFinishedMatch) {
          ...
          const isKo = matchData2.isKnockout || matchData2.stage && !/group/i.test(matchData2.stage);
          let addedTime = 0;
      ...
      function renderNewsList(data) {
        ...
        const renderItem = (item) => {
          ...
          const titleHtml = url ? `<a href="${url}" target="_blank" rel="noopener noreferrer" class="font-bold text-xs mb-1 block text-blue-100 hover:text-blue-300 hover:underline">${title}</a>` : `<div class="font-bold text-xs mb-1">${title}</div>`;
          ...
          const tags = item.tags?.length > 0 ? `<div class="flex flex-wrap gap-1 mt-2">${item.tags.map((tag) => `<span class="bg-white/5 px-1.5 py-0.5 rounded text-[11px] text-gray-500">${esc(tag)}</span>`).join("")}</div>` : "";
        ...
        const updatedLabel = Number.isFinite(updatedAt.getTime()) ? updatedAt.toLocaleString(state.uiLang === "en" ? "en-US" : "zh-CN") : tx("\u672A\u77E5", "Unknown");
        const body = news.length > 0 ? news.map(renderItem).join("") : `<div class="glass-light rounded-lg p-4 text-center"><div class="text-gray-500 text-xs">${emptyMessage}</div></div>`;
        return `<div class="space-y-3"><div class="flex items-center justify-between"><div class="flex items-center gap-2"><span class="text-lg">\u{1F4F0}</span><div><div class="text-sm font-bold">${tx("\u6BD4\u8D5B\u76F8\u5173\u65B0\u95FB", "Match News")}</div><div class="text-[11px] text-gray-500">${esc(displayMaybeTeamName(data.homeNameI18n || data.homeTeam || ""))} ${tx("\u5BF9\u9635", "vs")} ${esc(displayMaybeTeamName(data.awayNameI18n || data.awayTeam || ""))}</div></div></div><div class="text-[11px] text-gray-600">${tx("\u6765\u6E90", "Source")}: ${sourceLabel}</div></div>${body}<div class="text-[11px] text-gray-600 text-center">${tx("\u5171", "Total")} ${news.length} ${tx("\u6761\u65B0\u95FB", "news items")} \xB7 ${tx("\u66F4\u65B0\u65F6\u95F4", "Updated")}: ${updatedLabel}</div></div>`;
      }
      ...
      function renderH2HMatchList(matches) {
        ...
        return '<div class="space-y-1">' + matches.map((m) => {
          ...
          const teams = [displayMaybeTeamName(m.homeTeamNameI18n || m.homeTeamName), displayMaybeTeamName(m.awayTeamNameI18n || m.awayTeamName)].filter(Boolean).join(" vs ");
      ...
      function renderVenueWeather(data) {
        ...
        const grassDisplay = v.grass || (meta?.surfaceI18n ? window.WorldCup.I18n.i18nText(meta.surfaceI18n, meta.surface || "") : meta?.surface) || tx("\u672A\u77E5", "N/A");
        ...
        const weatherBlock = w ? `<div class="glass-light rounded-lg p-3"><div class="flex items-center justify-between mb-2"><span class="text-xs font-bold text-gray-400">${weatherIcon} ${tx("\u5929\u6C14\u72B6\u51B5", "Weather")}</span><span class="text-[11px] text-gray-500">${esc(w.description) || ""}</span></div><div class="grid grid-cols-3 gap-3 text-center"><div><div class="text-xl font-bold">${esc(w.temp) || "-"}\xB0C</div><div class="text-[11px] text-gray-500">${tx("\u6E29\u5EA6", "Temp")}</div><div class="text-[11px] text-gray-600">${tx("\u4F53\u611F", "Feels")} ${esc(w.feelsLike) || "-"}\xB0C</div></div><div><div class="text-xl font-bold">${esc(w.humidity) || "-"}%</div><div class="text-[11px] text-gray-500">${tx("\u6E7F\u5EA6", "Humidity")}</div></div><div><div class="text-xl font-bold">${w.windSpeed ? esc(Math.round(w.windSpeed)) : "-"}</div><div class="text-[11px] text-gray-500">${tx("\u98CE\u901F", "Wind")} km/h</div></div></div></div>` : `<div class="glass-light rounded-lg p-3"><div class="text-center text-gray-500 text-xs"><div class="mb-1">\u{1F324}\uFE0F ${tx("\u5929\u6C14\u6570\u636E", "Weather")}</div><div>${tx("\u6682\u65E0\u5B9E\u65F6\u5929\u6C14", "No weather data")}</div></div></div>`;
        const impactBlock = impact ? `<div class="glass-light rounded-lg p-3"><div class="text-xs font-bold text-gray-400 mb-2">\u{1F4CB} ${tx("\u573A\u5730\u4E8B\u5B9E", "Venue Facts")}</div>${impact.details?.map((d) => `<div class="text-[11px] text-gray-300 mb-1">\u2022 ${esc(d)}</div>`).join("") || ""}<div class="text-[10px] text-gray-600 mt-2">${esc(window.WorldCup.I18n.i18nText(impact.disclaimerI18n, ""))}</div></div>` : "";
        return `<div class="space-y-3"><div class="glass-light rounded-lg p-3">${venueBlock}</div>${weatherBlock}${impactBlock}</div>`;
      }
      ...
      function renderCornerAnalysis(data) {
        if (!data) return `<div class="text-gray-500 text-xs py-4 text-center">${tx("\u6570\u636E\u6682\u65E0", "No data")}</div>`;
        const p = data.predicted || {}, o = data.odds || {}, r = data.realtime || {}, h = data.historical || {}, v = data.verdict || {};
        const trend = v.trend || "neutral", trendEmoji = { over_strong: "\u{1F534}", over_slight: "\u{1F7E0}", under_strong: "\u{1F535}", under_slight: "\u{1F7E4}" }[trend] || "\u26AA";
        const conf = v.confidence === "high" ? "\u{1F7E2}" : v.confidence === "medium" ? "\u{1F7E1}" : "\u26AA";
        const hasLine = Number.isFinite(Number(o.line)), line = hasLine ? Number(o.line) : null;
        const progressBase = hasLine ? line : p.total || 1;
        const progressPct = Math.min(100, (r.current?.total || 0) / progressBase * 100), expectedPct = data.progress?.expected || 0;
        const lineLabel = o.kind === "reference" ? tx("\u53C2\u8003\u7EBF", "Reference") : tx("\u76D8\u53E3\u7EBF", "Market line");
        ...
        const lineDisplay = hasLine ? line : tx("\u6682\u65E0", "N/A");
        return `<div class="space-y-3"><div class="glass-light rounded-lg p-3"><div class="flex items-center justify-between mb-2"><span class="text-xs font-bold text-gray-400">\u{1F4D0} ${tx("\u89D2\u7403\u9884\u6D4B", "Corner Forecast")}</span><span class="text-xs text-gray-500">${lineLabel} <span class="font-bold text-white">${lineDisplay}</span></span></div><div class="flex items-center gap-3 mb-3"><div class="text-center"><div class="text-2xl font-bold text-white">${p.total ?? "-"}</div><div class="text-[11px] text-gray-500">${tx("\u9884\u6D4B\u603B\u89D2\u7403", "Projected Corners")}</div><div class="text-[9px] text-gray-600">${tx("\u5B9E\u9A8C\u6027\u4F30\u7B97", "Experimental estimate")}</div></div><div class="flex-1"><div class="flex items-center gap-1 mb-1"><span class="text-xs">${trendEmoji}</span><span class="text-xs font-bold ${trend.includes("over") ? "text-red-400" : trend.includes("under") ? "text-blue-400" : "text-gray-400"}">${esc(trend.replace("_", " ").toUpperCase())}</span><span class="ml-auto">${conf}</span></div><div class="text-[11px] text-gray-500">${tx("\u5B9E\u9645", "Actual")} <span class="font-bold text-white">${r.current?.total || 0}</span>${hasLine ? ` / ${line}` : ""}</div></div></div><div class="relative h-4 bg-white/5 rounded-full overflow-hidden mb-2"><div class="absolute top-0 bottom-0 w-0.5 bg-yellow-500/50" style="left:${expectedPct}%"></div><div class="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-700" style="width:${progressPct}%"></div><div class="absolute inset-0 flex items-center justify-between px-2 text-[9px]"><span class="text-white font-bold">${r.current?.total || 0}</span><span class="text-gray-400">${Math.round(progressPct)}%</span></div></div><div class="flex items-center justify-between text-[11px]"><span class="text-gray-500">${tx("\u672C\u5C4A\u6570\u636E\uFF0C\u5411\u8D5B\u4E8B\u5747\u503C\u6536\u7F29", "Tournament data, mean-shrunk")}</span><span class="${paceColor} font-bold">${paceStatus}</span></div></div><div class="grid grid-cols-2 gap-2"><div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">\u{1F535} ${tx("\u4E3B\u961F", "Home")}</div><div class="text-sm font-bold">${h.homeAvg ?? "-"} ${tx("\u573A\u5747\u89D2\u7403", "corners for")}</div><div class="text-[11px] text-gray-600">${tx("\u573A\u5747\u88AB\u89D2\u7403", "conceded")} ${h.homeAgainstAvg ?? "-"} \xB7 ${h.homeSampleCount || 0}${tx("\u573A", " matches")}</div></div><div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">\u{1F534} ${tx("\u5BA2\u961F", "Away")}</div><div class="text-sm font-bold">${h.awayAvg ?? "-"} ${tx("\u573A\u5747\u89D2\u7403", "corners for")}</div><div class="text-[11px] text-gray-600">${tx("\u573A\u5747\u88AB\u89D2\u7403", "conceded")} ${h.awayAgainstAvg ?? "-"} \xB7 ${h.awaySampleCount || 0}${tx("\u573A", " matches")}</div></div></div>${v.reason || v.reasonI18n ? `<div class="glass-light rounded-lg p-2"><div class="text-[11px] text-gray-500 mb-1">\u{1F4CA} ${tx("\u5206\u6790\u7ED3\u8BBA", "Verdict")}</div><div class="text-xs text-gray-300">${esc(window.WorldCup.I18n.i18nText(v.reasonI18n, v.reason || ""))}</div></div>` : ""}</div>`;
      }
  ...
  "static/js/team-detail.js"() {
    (function() {
      ...
      function renderTeamEnhanced(d, group, wcMatchData) {
        ...
        if (d.coach) {
          h += `<div style="${card}"><div style="${secHdr}">\u{1F9E0} ${tx("\u6559\u7EC3", "Coach")}</div><div style="font:500 13px/1 'Inter';color:rgba(248,250,252,.7);margin-bottom:2px">${translateCoachField(d.coach.name, "name")} <span style="font:400 10px/1 'Inter';color:rgba(248,250,252,.25)">${translateCoachField(d.coach.nationality, "nationality")}</span></div><div style="font:400 11px/1 'Inter';color:rgba(248,250,252,.35);margin-bottom:10px">${translateCoachField(d.coach.style, "style") || ""}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font:400 11px/1 'Inter'"><div><span style="color:rgba(248,250,252,.25)">${tx("\u6267\u6559\u65F6\u957F", "Tenure")}</span><span style="font-weight:600;margin-left:4px;color:rgba(248,250,252,.6)">${translateCoachField(d.coach.tenure || d.coach.since || "?", "tenure")}</span></div><div><span style="color:rgba(248,250,252,.25)">${tx("\u80DC\u7387", "Win Rate")}</span><span style="font-weight:600;margin-left:4px;color:#34d399">${d.coach.winRate || "?"}</span></div><div><span style="color:rgba(248,250,252,.25)">${tx("\u5E38\u7528\u9635\u578B", "Formation")}</span><span style="font-weight:600;margin-left:4px;color:rgba(248,250,252,.6)">${(d.coach.formation || []).join(" / ") || "?"}</span></div><div><span style="color:rgba(248,250,252,.25)">${tx("\u5927\u8D5B\u7ECF\u9A8C", "Tournament Exp.")}</span><span style="font-weight:600;margin-left:4px;color:rgba(248,250,252,.6)">${d.coach.bigTournament ? "\u2713" : "?"}</span></div></div></div>`;
        }
        if (d.squadChanges) {
    ...
    (function() {
      ...
      function renderFrozenKnockoutComparison(review) {
        ...
        if (!sections) {
          ...
          })[tag] || { zh: tag, en: tag }).map((tag) => i18nText(tag, "")).join(" \xB7 ") || tx("\u8986\u76D6\u4E0D\u8DB3", "Insufficient coverage");
      ...
      function renderMatchReview(review) {
        ...
        const hasLocalizedPostmortem = uiLang2 === "zh" ? Boolean(aiPostmortem.headlineI18n?.zh || postmortemItems.length) : Boolean(aiPostmortem.headlineI18n?.en || postmortemItems.length);
        ...
        const accLabel = bias.accuracy === "highly_accurate" || bias.accuracy === "exact_score" ? `\u{1F7E2} ${tx("\u7CBE\u51C6\u547D\u4E2D", "Accurate")}` : bias.accuracy === "result_correct_score_wrong" ? `\u{1F7E1} ${tx("\u6BD4\u5206\u504F\u5DEE", "Score off")}` : bias.accuracy === "regulation_draw_advancer_correct" ? `\u{1F535} ${tx("\u547D\u4E2D\u664B\u7EA7\u65B9", "Advancer hit")}` : bias.accuracy === "wrong_result" ? `\u{1F534} ${tx("\u7ED3\u679C\u9519\u8BEF", "Wrong result")}` : `\u26AA ${tx("\u672A\u77E5", "Unknown")}`;
  ...
  "static/js/mr-hud.js"() {
    ...
    (() => {
      ...
      function renderHudVenuePanel(venueData) {
        ...
        const grass = v.grass || (meta?.surfaceI18n ? window.WorldCup.I18n.i18nText(meta.surfaceI18n, meta.surface || "") : meta?.surface) || tx("\u672A\u77E5", "N/A");
