...
const logger = createLogger('team-data');
const fs = require('fs');
...
function init({ loader, teamResolver, idBridge, PLAYER_RATINGS }) {
  ...
  try {
    ...
    logger.warn('⚠️ id_map_center.json not loaded:', { error: e.message });
  }
  ...
  logger.info(`✅ Populated TEAM_NAMES: ${Object.keys(TEAM_NAMES).length} teams`);
  ...
  try {
    ...
    logger.info(`🖼️ Loaded team logos: ${Object.keys(TEAM_LOGOS).length} teams`);
  } catch (e) {
    logger.info('⚠️ team_logos.json not found');
  }
  ...
  try {
    ...
    logger.info(`📊 Built Elo rank map: ${Object.keys(ELO_RANK_MAP).length} teams`);
  } catch (e) {
    logger.info('⚠️ Could not build Elo rank map:', { error: e.message });
  }
}
