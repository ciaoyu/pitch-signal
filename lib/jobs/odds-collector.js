...
function createOddsCollectorJob(deps) {
  ...
  async function collectOdds(options = {}) {
    ...
    if (!options.force && dailyCalls >= MAX_DAILY_CALLS) {
      const quotaMsg = `Odds collector quota exhausted (${dailyCalls}/${MAX_DAILY_CALLS} daily calls). Skipping non-milestone collection.`;
      console.log(`⏸️ ${quotaMsg}`);
      return { status: 'quota_exhausted', dailyCalls, maxDailyCalls: MAX_DAILY_CALLS };
    }
    ...
    try {
      ...
      return { status: 'success', recordedCount, dailyCalls };
    } catch (e) {
  ...
  return {
    ...
    collectOdds,
    getQuotaStatus() {
      return { dailyCalls, maxDailyCalls: MAX_DAILY_CALLS, remaining: Math.max(0, MAX_DAILY_CALLS - dailyCalls) };
    }
  };
}
