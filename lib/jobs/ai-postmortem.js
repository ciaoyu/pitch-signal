...
const SCHEMA_BACKFILL_MS = 15 * 60 * 1000;
const SCHEMA_BACKFILL_BATCH = 2;

async function refreshOutdatedReviews(db, reviewService, schemaVersion) {
  if (!reviewService?.reviewMatch || !schemaVersion) return 0;
  const rows = db.prepare(
    "SELECT match_id, review_json FROM post_match_reviews WHERE status='completed' ORDER BY updated_at DESC"
  ).all();
  let refreshed = 0;
  const outdatedRows = rows.filter(row => {
    let review = null;
    try { review = JSON.parse(row.review_json); } catch { review = null; }
    return review?.schemaVersion !== schemaVersion;
  }).slice(0, SCHEMA_BACKFILL_BATCH);
  for (const row of outdatedRows) {
    try {
      const rebuilt = await reviewService.reviewMatch(String(row.match_id), { persist: true });
      if (rebuilt?.schemaVersion === schemaVersion) refreshed++;
    } catch (error) {
      console.warn(`AI postmortem schema refresh skipped for ${row.match_id}: ${error.message}`);
    }
  }
  return refreshed;
}

function hasOutdatedReviews(db, schemaVersion) {
  const rows = db.prepare(
    "SELECT review_json FROM post_match_reviews WHERE status='completed'"
  ).all();
  return rows.some(row => {
    try { return JSON.parse(row.review_json)?.schemaVersion !== schemaVersion; }
    catch { return true; }
  });
}
...
function createBilingualFallback(review, error) {
  ...
  const homeScore = Number(match.regulationScore?.home ?? match.homeScore ?? match.home?.score ?? 0);
  const awayScore = Number(match.regulationScore?.away ?? match.awayScore ?? match.away?.score ?? 0);
  ...
  return {
    ...
    whyRightI18n: {
      zh: [`常规时间结果为${actualScore}，赛前模型预测比分为${predictedScore}。`],
      en: [`The 90-minute result was ${actualScore}; the pre-match model projected ${predictedScore}.`],
    },
    ...
    processNotesI18n: {
      zh: [`本复盘依据常规时间比分、最终比分、赛前预测及${eventCount}条事件、${commentaryCount}条文字直播生成。`],
      en: [`This review uses the regulation and final scores, the pre-match snapshot, ${eventCount} events and ${commentaryCount} commentary entries.`],
    },
...
function createAiPostmortemJob(deps) {
  ...
  function nextAnalysisDelay(dataDir) {
    ...
    try {
      const { db } = require('../db');
      const { REVIEW_SCHEMA_VERSION } = require('../postMatchReview');
      if (hasOutdatedReviews(db, REVIEW_SCHEMA_VERSION)) {
        console.log(`🤖 AI postmortem: schema backfill continues in ${SCHEMA_BACKFILL_MS / 60000}min`);
        return SCHEMA_BACKFILL_MS;
      }
  ...
  async function runAndSchedule(dataDir) {
    ...
    try {
      const refreshed = await refreshOutdatedReviews(db, deps.reviewService, REVIEW_SCHEMA_VERSION);
      if (refreshed) console.log(`🤖 AI postmortem: rebuilt ${refreshed} outdated review(s) for ${REVIEW_SCHEMA_VERSION}`);
      const rows = db.prepare(
          ...
          try {
            ...
            const ctx = review.aiPromptContext?.requiredOutputFormat
              ...
              : {
                  ...
                  liveTimelineI18n: review.liveTimelineI18n || [],
...
module.exports = { createAiPostmortemJob, refreshOutdatedReviews, hasOutdatedReviews };
