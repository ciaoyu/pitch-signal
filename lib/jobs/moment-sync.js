...
function countRedCards(events, teamEspnId) {
  ...
  let reds = 0;
  const yellowsByAthlete = new Map();

  for (const evt of events) {
    const team = evt?.team?.id ?? '';
    if (String(team) !== tid) continue;

    const type = evt?.type?.text ?? '';
    const athleteId = evt?.athletesInvolved?.[0]?.id ?? evt?.participants?.[0]?.athlete?.id ?? '';

    if (/red/i.test(type)) {
      reds++;
    } else if (/yellow/i.test(type) && athleteId) {
      const prev = yellowsByAthlete.get(String(athleteId)) || 0;
      yellowsByAthlete.set(String(athleteId), prev + 1);
      if (prev + 1 >= 2) reds++;
    }
  }
