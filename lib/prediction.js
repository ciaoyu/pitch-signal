...
/**
 * Model version + config contract (P0 quarantine, Owner A v2).
 * This version MUST change whenever the active signal set, the home-advantage
 * semantics, or any probability-forming parameter changes, so that no future
 * public probability can be mistaken for the pre-quarantine model.
 * @see docs/prediction-system-remediation-master-plan-2026-07.md §5 (Owner A)
 */
const MODEL_VERSION = 'p0-quarantine-v3-2026-07-10';
// Active pre-match signal set after quarantine: Elo + Poisson ONLY form the
// public probability. odds / marketValue / continentalStrength remain opt-in
// via the service layer (env-gated) and degrade to zero weight.
// coach / capacity-venue / fatigue are REMOVED from the core (usedInModel:false).
const ACTIVE_SIGNALS = ['elo', 'poisson'];
// Manually quarantined signals (present in old code, removed from probability).
const QUARANTINED_SIGNALS = ['coach', 'venue', 'fatigue'];
...
class PredictionEngine {
  constructor(options = {}) {
    ...
    // P0 quarantine (Owner A v2): coach / capacity-venue / fatigue are REMOVED
    // from the core probability. They are kept at weight 0 and excluded from the
    // fusion array (see §2.2.2/§2.2.3 and the rejection notes). odds /
    // marketValue / continentalStrength are opt-in (env-gated) and default to
    // zero weight (their signal objects are null unless explicitly enabled).
    // P0 quarantine v3 (Owner A): the PUBLIC probability is formed ONLY from the
    // active signals (elo + poisson). odds / marketValue / continentalStrength are
    // candidate-only: computed for transparency but EXCLUDED from the fusion
    // array, so they can NEVER change the public probability — even if a caller
    // passes them and even if the relevant env gate is open. Their weights are
    // therefore 0 in the P0 public contract (see p0PublicFusionExcludes).
    ...
    // Load coach data — retained for DISPLAY / intelligence cards only.
    // It is NOT used to form any public probability (usedInModel: false).
    try {
  ...
  /** Immutable model version for the audit ledger / snapshot. */
  get modelVersion() { return MODEL_VERSION; }

  /**
   * Complete model contract — the single source of truth for what forms the
   * public probability. This is hashed into configHash. It must cover EVERY
   * probability-forming element (active signals, weights, rho, env params,
   * gate policies) so the version contract cannot be impersonated and so a
   * diff of two contracts is a complete model diff.
   */
  get modelContract() {
    return {
      modelVersion: MODEL_VERSION,
      activeSignals: ACTIVE_SIGNALS.slice(),
      quarantinedSignals: QUARANTINED_SIGNALS.slice(),
      // P0 quarantine v3: the ONLY signals that form the public probability.
      publicFusionSignals: ACTIVE_SIGNALS.slice(),
      // Signals computed but EXCLUDED from the public fusion (candidate / shadow
      // only). They can never move the public number in this P0 model.
      p0PublicFusionExcludes: ['odds', 'marketValue', 'continentalStrength'],
      // Candidate (non-fused) signals — present for transparency/audit, marked
      // usedInModel:false. Re-enabled only by a future, separately approved phase.
      candidates: {
        odds: { usedInModel: false, status: 'candidate_shadow', gate: 'env-gated:THE_ODDS_API_KEY' },
        marketValue: { usedInModel: false, status: 'candidate_shadow', gate: 'env-gated:MARKET_VALUE_SIGNAL_ENABLED' },
        continentalStrength: { usedInModel: false, status: 'candidate_shadow', gate: 'env-gated:CONTINENTAL_STRENGTH_SIGNAL_ENABLED' },
      },
      // Full weight table (incl. opt-in / quarantined) — changing any weight
      // busts the hash.
      baseWeights: Object.assign({}, this.baseWeights),
      elo: { homeAdvantage: this.elo.homeAdvantage },
      poisson: {
        homeAdvantage: this.poisson.homeAdvantage,
        globalAvgGoals: this.poisson.globalAvgGoals,
        rho: this.poisson.rho,
      },
      // Environmental hand-tuned β coefficients (P0: QUARANTINED to shadow).
      envFactors: {
        status: 'shadow_only',
        appliedInModel: false,
        owner: 'E',
        altitudeCoef: 0.03,
        heatCoef: 0.01,
        heatThresholdC: 32,
      ...
      // Manual KO λ shrinkage removed from probability (Owner A v2 rejection).
      knockoutShrinkage: { quarantined: true, appliedInModel: false },
      // Signal gates — static policy declarations (the full contract must
      // record these so a change in gate policy busts the hash).
      gates: {
        polymarket: false, // always closed in public Beta
        odds: 'env-gated:THE_ODDS_API_KEY',
        marketValue: 'env-gated:MARKET_VALUE_SIGNAL_ENABLED',
        continentalStrength: 'env-gated:CONTINENTAL_STRENGTH_SIGNAL_ENABLED',
        aiContext: false, // AI context is shadow-only, never enters probability
      },
      // Neutral-venue rule (Owner A v2): all World Cup matches close the
      // nominal home advantage; host effect size waits for data estimation.
      neutralVenueRule: { closeNominalHomeAdvantage: true, hostEffectEstimatedBy: 'pending' },
    };
  }

  /** Deterministic config hash (SHA-256, 12 hex) over the full model contract. */
  get configHash() {
    return crypto.createHash('sha256').update(JSON.stringify(this.modelContract)).digest('hex').slice(0, 12);
  }
  ...
  predict(params) {
    ...
    // ============================================
    // 0. Neutral venue / host determination (P0 quarantine, Owner A v2)
    // ============================================
    // World Cup venues are neutral. Per the rejection ruling, the P0 rule is:
    //   - Close the nominal home advantage for ALL World Cup matches
    //     (neutralVenue === true => applyHome = false, regardless of which side
    //     is the host nation). The "host" is recorded as a FACT only
    //     (hostSide), never as a probability bonus. Host effect SIZE waits for
    //     data estimation (Owner E). This also correctly handles the case where
    //     the host nation is the nominal away team.
    // For non-World-Cup / league contexts (neutralVenue falsy) the legacy home
    // advantage is preserved.
    const neutralVenue = Boolean(params.neutralVenue);
    const hostTeamId = params.hostTeamId || null;
    const applyHome = !neutralVenue;
    const hostSide = hostTeamId
      ? (params.homeId === hostTeamId ? 'home' : params.awayId === hostTeamId ? 'away' : 'none')
      : 'none';
    const eloHomeAdvantage = applyHome ? this.elo.homeAdvantage : 0;
    ...
    const homeRawLambda = this.poisson.calculateLambda(
      ...
      true,
      { isKnockout: params.isKnockout, knockoutRound: params.knockoutRound, opponentIsEliteDefense: awayRating?.isEliteDefense, applyHomeAdvantage: applyHome }
    );
    const awayRawLambda = this.poisson.calculateLambda(
      awayRating?.attack_strength || 1.0,
      homeRating?.defense_strength || 1.0,
      false,
      { isKnockout: params.isKnockout, knockoutRound: params.knockoutRound, opponentIsEliteDefense: homeRating?.isEliteDefense, applyHomeAdvantage: applyHome }
    );
    ...
    // P0 quarantine (Owner A v2): the hand-tuned altitude/heat β coefficients
    // (0.03, 0.01, 32°C) are NOT "researched / data-driven" — they are manual
    // priors pending Owner E's estimation. They must NOT alter λ. Keep them as
    // shadow/display only: homeLambdaShadow/awayLambdaShadow show the would-be
    // λ; the public probability is formed from the UN-adjusted λ.
    const homeLambdaShadow = homeLambda * homeEnvBeta;
    const awayLambdaShadow = awayLambda * awayEnvBeta;
    const homeLambdaAdj = homeLambda; // env NOT applied to probability
    const awayLambdaAdj = awayLambda;
    ...
    // --- Coach signal: QUARANTINED (P0, Owner A v2) ---
    // The coach model was generated from tenure constants + Chinese style keywords
    // and could carry a 0.30 confidence even with no data. It is excluded from the
    // core probability and kept only for DISPLAY. No weight, no component.
    // (usedInModel: false — see rejection notes). The helper methods
    // (calcCoachFactor / calcCoachConfidence / _parseCoachStyle / _parseTenure)
    // are retained for any display card but are no longer called here.

    // --- Venue signal: capacity leg QUARANTINED (P0, Owner A v2) ---
    // The old capacity->home-advantage transform has nothing to do with real
    // environmental adaptation and is removed from the core. The data-driven
    // altitude/heat envFactor (venueFactors.js) is a separate, researched
    // feature and is likewise quarantined to shadow/display in this P0 (below).
    ...
    // --- Fatigue index (KO-5): QUARANTINED from probability (P0, Owner A v2) ---
    // The fatigue signal (weight 0.04, maxTilt 0.025) was merged into main but
    // its weight/tilt were never OOS-estimated. It must NOT form any public
    // probability. The knockout-intel `fatigue` SECTION remains display-only
    // (built independently in services/knockout-intel.js) but is no longer fed
    // into the engine. (usedInModel: false)

    // ============================================
    // 2. Confidence dynamic weighting
    // ============================================
    // P0 quarantine: coach / venue / fatigue are absent from the array (missing
    // signals must be absent, not zero-weighted for show — master plan §1/§5).
    // P0 quarantine v3 (Owner A): the public fusion uses ONLY the active signals
    // (elo + poisson). odds / marketValue / continentalStrength are intentionally
    // absent from this array — computed below for transparency (see `candidates`)
    // but NEVER fused, so passing them cannot alter the public probability. This
    // closes the v2 review blocker #1.
    const signals = ACTIVE_SIGNALS.map((name) => {
      if (name === 'elo') return { name, norm: eloNorm, confidence: eloConfidence };
      if (name === 'poisson') return { name, norm: poissonNorm, confidence: poissonConfidence };
      return null;
    }).filter(Boolean);
    ...
    // ============================================
    // 4. Polymarket / Market Odds Quarantine (usedInModel: false)
    // ============================================
    // Per P0 contract & Owner G review: market signals (including polymarketOdds)
    // are strictly prohibited from altering public Elo + Poisson probabilities.
    // They are archived as shadow candidates only.
    ...
    if (params.polymarketOdds) {
      ...
      marketProb = pmNorm;
      // Do NOT modify fused.home / fused.draw / fused.away
      polymarketApplied = false;
      conflictResolved = 'quarantined_shadow_only';
    }
    ...
    return {
      ...
      components: {
        ...
        poisson: {
          ...poissonNorm, homeLambda, awayLambda, homeLambdaAdj, awayLambdaAdj,
          // P0 quarantine: env β is shadow-only — never applied to probability.
          envShadow: { homeLambdaShadow, awayLambdaShadow, homeBeta: homeEnvBeta, awayBeta: awayEnvBeta },
          envApplied: false,
          envAppliedInModel: false,
          valid: poissonValid, confidence: poissonConfidence, dixonColes: poissonPred.dixonColes,
        },
        // P0 quarantine: coach / capacity-venue / fatigue are intentionally
        // absent. They must NOT appear here with any (even zero) weight.
        // (usedInModel: false)
        // P0 quarantine v3: candidate signals are present for transparency but are
        // NOT fused (usedInModel:false, status:'candidate').
        odds: oddsFactor ? { ...oddsNorm, confidence: oddsConfidence, usedInModel: false, status: 'candidate' } : null,
        marketValue: marketValueSignal ? { ...marketValueNorm, confidence: marketValueConfidence, usedInModel: false, status: 'candidate', meta: marketValueSignal } : null,
        continentalStrength: continentalStrengthSignal ? { ...continentalStrengthNorm, confidence: continentalStrengthConfidence, usedInModel: false, status: 'candidate', meta: continentalStrengthSignal } : null,
      },
      // Environmental factor (altitude/heat): λ' = λ · β_alt · β_temp.
      // P0 quarantine (Owner A v2): the hand-tuned β coefficients are SHADOW
      // only — displayed for transparency/audit but NEVER used to form the
      // public probability. Estimates belong to Owner E.
      venueFactor: envFactor && !envFactor.error ? {
        ...
        home: envFactor.home,
        away: envFactor.away,
        shadowLambda: { home: Math.round(homeLambdaShadow * 1000) / 1000, away: Math.round(awayLambdaShadow * 1000) / 1000 },
      } : (envFactor?.error ? { applied: false, appliedInModel: false, error: envFactor.error } : { applied: false, appliedInModel: false }),
      weights: dynamicWeights,
      // Candidate (non-fused) signals — aggregated with an explicit
      // usedInModel:false guarantee. The public probability (homeWin/draw/awayWin)
      // is formed WITHOUT these.
      candidates: {
        odds: oddsFactor ? { ...oddsNorm, confidence: oddsConfidence, usedInModel: false, status: 'candidate' } : null,
        polymarket: marketProb ? { ...marketProb, usedInModel: false, status: 'candidate_shadow' } : null,
        marketValue: marketValueSignal ? { ...marketValueNorm, confidence: marketValueConfidence, usedInModel: false, status: 'candidate' } : null,
        continentalStrength: continentalStrengthSignal ? { ...continentalStrengthNorm, confidence: continentalStrengthConfidence, usedInModel: false, status: 'candidate' } : null,
      },
      marketSignalFusion: { applied: false, usedInModel: false, marketProb: marketProb || null },
      // --- Knockout target semantics split (P0, Owner A v2) ---
      // The public homeWin/draw/awayWin above IS the 90-minute (regulation)
      // result. 90-minute result and final advancement are NEVER conflated
      // (master plan red line). The advance block is a separate, explicit target.
      // No OOS-estimated extra-time / penalty model exists in this quarantine,
      // so advance is display-only and NOT blended into any public number.
      // When unavailable it MUST NOT return pseudo probabilities — it returns
      // home: null / away: null (rejection note: 50/50 split was self-contradictory).
      regulation: { homeWin: fused.home, draw: fused.draw, awayWin: fused.away },
      advance: params.isKnockout
        ? {
            ...
            usedInModel: false,
            available: false,
            reason: 'No OOS-estimated extra-time/penalty model in P0 quarantine; advance probability is display-only and unavailable (returns null, not a pseudo 50/50).',
            method: 'unavailable_null',
          }
        : null,
      knockout: params.isKnockout
        ? { isKnockout: true, knockoutRound: params.knockoutRound || null, lambdaShrinkageQuarantined: true }
        : { isKnockout: false },
      host: neutralVenue
        ? { neutralVenue: true, hostTeamId, hostSide, applyHome, applyHomeAdvantage: false }
        : { neutralVenue: false, applyHome },
      quarantinedSignals: QUARANTINED_SIGNALS.slice(),
      modelVersion: MODEL_VERSION,
      modelContract: this.modelContract,
      configHash: this.configHash,
      _fusion: 'v8_p0_quarantine_v3',
      // Heuristic uncertainty spread (P0 quarantine v3, blocker #3): this is a
      // HAND-TUNED heuristic (signal divergence / sample size / lineup uncertainty
      // / knockout flag). It is NOT a statistically valid confidence interval and
      // must NOT be presented with "95% CI" / coverage semantics. A proper
      // empirical coverage interval is owned by Owner D. status:'unvalidated'.
      heuristicUncertainty: this.calcHeuristicUncertainty({
        homeWin: fused.home,
        draw:    fused.draw,
        awayWin: fused.away,
        eloNorm,
  ...
  /**
   * Heuristic uncertainty spread (P0 quarantine v3 — Owner A blocker #3).
   *
   * ⚠️ This is NOT a statistically valid confidence interval. It is a hand-tuned
   * heuristic that widens a band around the point estimate based on signal
   * divergence, xG sample size, lineup uncertainty, and knockout flag. It must
   * NOT be surfaced as a "95% CI" or given coverage semantics. A proper empirical
   * coverage interval is owned by Owner D (hence status:'unvalidated').
   *
   * Output [lo, hi] centered on point estimate, clamped lo>=0, hi<=1.
   * band: 'high' | 'medium' | 'low' (heuristic label, NOT a CI level).
   */
  calcHeuristicUncertainty({ homeWin, draw, awayWin, eloNorm, poissonNorm, xgMatchesPlayed, lineupUncertainty, isKnockout }) {
    ...
    const band = halfWidth < 0.08 ? 'high' : halfWidth < 0.13 ? 'medium' : 'low';
