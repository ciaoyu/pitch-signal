...
class PredictionEngine {
  constructor(options = {}) {
    ...
    // 基础权重（会被置信度动态调整）
    this.baseWeights = options.weights || {
      elo: 0.30,
      poisson: 0.25,
      coach: 0.15,
      venue: 0.10,
   ...
   */
  predict(params) {
    const { homeId, awayId, homeRating, awayRating, odds, venue } = params;
    ...
    const poissonPred = this.poisson.predictMatchWithLambda(homeLambda, awayLambda);
    ...
    // --- 教练信号 ---
    const coachFactor = this.calcCoachFactor(homeId, awayId);
    const coachConfidence = this.calcCoachConfidence(homeId, awayId);
    ...
    // ============================================
    // 最可能比分
    const likelyScore = { score: poissonPred.likelyScore, prob: poissonPred.likelyScoreProb };

    return {
      homeWin: fused.home,
      draw: fused.draw,
      awayWin: fused.away,
      likelyScore: likelyScore.score,
      likelyScoreProb: Math.round(likelyScore.prob * 1000) / 1000,
      goals: {
        homeExpected: Math.round(homeLambda * 10) / 10,
        awayExpected: Math.round(awayLambda * 10) / 10,
      },
      components: {
        elo: { ...eloNorm, confidence: eloConfidence },
        poisson: { ...poissonNorm, homeLambda, awayLambda, valid: poissonValid, confidence: poissonConfidence, dixonColes: poissonPred.dixonColes },
  ...
  calcCoachConfidence(homeId, awayId) {
    ...
    if (homeCoach?.experience > 5) conf += 0.10;
    if (awayCoach?.experience > 5) conf += 0.10;
  ...
  calcCoachFactor(homeId, awayId) {
    ...
    if (homeCoach) {
      homeAdjust = 0.5 + (homeCoach.experience || 0) * 0.002;
      if (homeCoach.style === 'attacking') homeAdjust += 0.02;
      if (homeCoach.style === 'defensive') homeAdjust -= 0.01;
    }
    if (awayCoach) {
      awayAdjust = 0.5 + (awayCoach.experience || 0) * 0.002;
      if (awayCoach.style === 'attacking') awayAdjust += 0.01;
      if (awayCoach.style === 'defensive') awayAdjust -= 0.02;
    }
  ...
  calcOddsFactor(odds) {
    ...
    const raw = (1 / homeDec) + (1 / drawDec) + (1 / awayDec);
    return {
      homeWinProb: Math.round(((1 / homeDec) / raw) * 1000) / 1000,
      drawProb: Math.round(((1 / drawDec) / raw) * 1000) / 1000,
      awayWinProb: Math.round(((1 / awayDec) / raw) * 1000) / 1000,
    };
  }
}
