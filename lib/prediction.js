...
class PredictionEngine {
  ...
  eloGuidedLambda(eloWinProb, rawLambda, eloRating, oppRating) {
    ...
    // 与 Poisson 原始计算共享同一"联赛平均进球"基准，避免用孤立的过期常数
    // (globalAvgGoals 曾从 2.5 修复为 1.2，这里之前遗漏未同步)
    const baseLambda = this.poisson.globalAvgGoals;
  ...
  predict(params) {
    ...
    // ============================================
    // Display score: per-team Poisson mode (期望比分).
    // For a Poisson(λ) variable the mode is floor(λ) whenever λ is not an
    // integer (e.g. λ=1.95 still has mode 1, since P(1) > P(2) up to λ=2) —
    // Math.round(λ) does not match this and was inflating low-λ sides by one
    // goal (e.g. λ=0.6 → true mode 0, but round(0.6)=1), which flattened
    // lopsided matchups into the same generic score as close ones.
    //
    // If floor(λ) produces a draw but one team has >60% win probability,
    ...
    // before flooring to integer, so e.g. rawLambda=1.47 → display=1.5 → floor=1
    // stays consistent with the goals display (rounded to 1 decimal first).
    const dispHome = Math.round(homeLambdaAdj * 10) / 10;
    const dispAway = Math.round(awayLambdaAdj * 10) / 10;
    let expHome = Math.floor(dispHome);
    let expAway = Math.floor(dispAway);
