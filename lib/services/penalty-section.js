...
function buildTeamPenaltyContext(teamId, db) {
  ...
  if (shootouts > 0) notes.push(`${won}/${shootouts} shootout wins in this tournament`);
  ...
  return {
    ...
    shootoutExperience: experienceLabel(shootouts),
    shootouts,
    shootoutsWon: won,
...
function buildPenaltySection(ctx = {}) {
  ...
  return {
    ...
    source: 'ratings+schedule+player-events',
    usedInModel: false,
    likelihood,
    home: {
      name: homeName || null,
