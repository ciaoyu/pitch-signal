...
function strongerSide(home, away) {
  ...
  if (margin < 25) return {
    side: 'even',
    reason: 'Elo and experience roughly level',
    reasonI18n: { zh: '双方 Elo 与点球经验大致相当', en: 'Elo and experience roughly level' },
  };
  if (diff > 0) {
    const level = margin > 70 ? 'clear' : 'slight';
    return {
      side: 'home',
      reason: `Home side shows ${level} advantage in Elo/defence/shootout experience`,
      reasonI18n: {
        zh: `主队在 Elo、防守与点球大战经验上${level === 'clear' ? '优势明显' : '略占优势'}`,
        en: `Home side shows ${level} advantage in Elo/defence/shootout experience`,
      },
    };
  }
  const level = margin > 70 ? 'clear' : 'slight';
  return {
    side: 'away',
    reason: `Away side shows ${level} advantage in Elo/defence/shootout experience`,
    reasonI18n: {
      zh: `客队在 Elo、防守与点球大战经验上${level === 'clear' ? '优势明显' : '略占优势'}`,
      en: `Away side shows ${level} advantage in Elo/defence/shootout experience`,
    },
  };
}
