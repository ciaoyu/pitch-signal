...
function strongerBench(home, away) {
  ...
  if (Math.abs(diff) < 1) return {
    side: 'even',
    reason: 'Bench outcomes roughly level',
    reasonI18n: { zh: '双方替补登场后的比赛影响大致相当', en: 'Bench outcomes roughly level' },
  };
  return diff > 0
    ? {
        side: 'home',
        reason: 'Home bench produced the stronger post-substitution goal balance',
        reasonI18n: { zh: '主队替补登场后的进球净影响更强', en: 'Home bench produced the stronger post-substitution goal balance' },
      }
    : {
        side: 'away',
        reason: 'Away bench produced the stronger post-substitution goal balance',
        reasonI18n: { zh: '客队替补登场后的进球净影响更强', en: 'Away bench produced the stronger post-substitution goal balance' },
      };
}
