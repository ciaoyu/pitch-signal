...
class PredictionService {
  ...
  async predictMatch(matchId, { persist = false, includeExternalOdds = false, bypassCache = false } = {}) {
    ...
    // KO-5: fatigue index only makes sense for knockout fixtures (uses the
    // KO-1 shared infra: schedule-lookup, venue-distance, matches.went_to_et).
    const fatigueSignalValue = koCtx.isKnockout
      ? fatigueSignal.buildSignal({ matchId, homeId, awayId, db: dbInstance })
      : null;
    logger.debug('KO-5: fatigue signal', { matchId, isKnockout: koCtx.isKnockout, hasSignal: Boolean(fatigueSignalValue) });
    ...
    const result = aiEnabled
      ? await engine.predictWithAI({ matchId, homeId: homeLookup.ratingsId, awayId: awayLookup.ratingsId, homeEspnId: homeId, awayEspnId: awayId, homeRating, awayRating, odds, groupName, isKnockout: koCtx.isKnockout, knockoutRound: koCtx.knockoutRound, marketValueSignal: squadMarketValueSignal, continentalStrengthSignal: confedStrengthSignal, fatigueSignal: fatigueSignalValue })
      : await engine.predictWithMarket({ matchId, homeId: homeLookup.ratingsId, awayId: awayLookup.ratingsId, homeEspnId: homeId, awayEspnId: awayId, homeRating, awayRating, odds, venue, isKnockout: koCtx.isKnockout, knockoutRound: koCtx.knockoutRound, marketValueSignal: squadMarketValueSignal, continentalStrengthSignal: confedStrengthSignal, fatigueSignal: fatigueSignalValue });
