...
class PredictionService {
  ...
  async predictMatch(matchId, { persist = false, includeExternalOdds = false, bypassCache = false } = {}) {
    ...
    if (aiEnabled) {
      ...
      } catch { console.debug('PredictionService: group lookup failed for', homeId, 'vs', awayId); }
    }
    ...
    const result = aiEnabled
      ...
      : await engine.predictWithMarket({ matchId, homeId: homeLookup.ratingsId, awayId: awayLookup.ratingsId, homeEspnId: homeId, awayEspnId: awayId, homeRating, awayRating, odds });
    ...
    try {
      const probs = require('../../data/sources/seed/wc2026/probs.json');
  ...
  async getTeamElo(teamId) {
    ...
    } catch { console.warn('PredictionService.getTeamElo: DB lookup failed for', teamId); }
