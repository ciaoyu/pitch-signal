...
class PredictionService {
  ...
  async getEloRankings() {
    const { getCached, setCache, RATINGS, getTeamNameZh, getTeamNameI18n } = this.deps;
    const dbInstance = require('../db').db;
    ...
    for (const [id, team] of Object.entries(RATINGS.teams || {})) {
      if (team.rating) {
        teams[id] = { name: getTeamNameZh(id), nameI18n: getTeamNameI18n ? getTeamNameI18n(id, team.name) : null, rating: team.rating };
      } else {
        const players = team.players || [];
        if (players.length > 0) {
          const avg = players.reduce((s, p) => s + (p.rating || 70), 0) / players.length;
          teams[id] = { name: getTeamNameZh(id), nameI18n: getTeamNameI18n ? getTeamNameI18n(id, team.name) : null, rating: Math.round(avg * 10) + 1000 };
        }
      }
    }
  ...
  async getTeamElo(teamId) {
    const { RATINGS, getTeamNameZh, getTeamNameI18n } = this.deps;
    const dbInstance = require('../db').db;
    const teamResolver = require('../team_resolver');
