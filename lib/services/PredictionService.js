...
class PredictionService {
  ...
  async predictMatch(matchId, { persist = false, includeExternalOdds = false, bypassCache = false } = {}) {
    ...
    const predKey = includeExternalOdds ? `pred_odds_${matchId}` : `pred_${matchId}`;
    const cached = getCached(predKey, 300000);
    if (cached && !persist && !bypassCache) return cached;
...
module.exports.getKnockoutContextForMatch = getKnockoutContextForMatch;
