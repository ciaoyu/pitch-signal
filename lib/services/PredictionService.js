...
class PredictionService {
  ...
  calcAvg(team, source) {
    ...
    // 球员评分均值天然聚集在 70-90 区间,直接除以 75 会把全部球队压缩进 0.9~1.15,
    // 导致强弱队 λ 差异不足、比分众数在多场比赛间趋同。放大与基准的偏离幅度以拉开区分度。
    const STRENGTH_AMPLIFY = 3;
    const rawAtk = atkAvg / 75;
    const rawDef = defAvg / 75;
    return {
      rating: Math.round(avg * 10) + 1000,
      attack_strength: Math.round((1 + (rawAtk - 1) * STRENGTH_AMPLIFY) * 1000) / 1000,
      defense_strength: Math.round((1 + (rawDef - 1) * STRENGTH_AMPLIFY) * 1000) / 1000,
    };
  }
  ...
  async predictMatch(matchId, { persist = false, includeExternalOdds = false, bypassCache = false } = {}) {
    ...
    const result = aiEnabled
      ? await engine.predictWithAI({ matchId, homeId: homeLookup.ratingsId, awayId: awayLookup.ratingsId, homeEspnId: homeId, awayEspnId: awayId, homeRating, awayRating, odds, groupName })
      : await engine.predictWithMarket({ matchId, homeId: homeLookup.ratingsId, awayId: awayLookup.ratingsId, homeEspnId: homeId, awayEspnId: awayId, homeRating, awayRating, odds, venue });
