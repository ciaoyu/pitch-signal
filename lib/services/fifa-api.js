...
function parseLiveMatch(raw) {
  ...
  return {
    ...
    // FIFA's live payload is the source of truth for lineup/discipline facts.
    homeBookings: parseBookings(home.Bookings ?? []),
    awayBookings: parseBookings(away.Bookings ?? []),
    homeSubstitutions: parseSubstitutions(home.Substitutions ?? []),
    awaySubstitutions: parseSubstitutions(away.Substitutions ?? []),
    homeGoals: parseGoals(home.Goals ?? []),
    awayGoals: parseGoals(away.Goals ?? []),
    possession: raw.BallPossession ?? null,
  };
}
...
function parseBookings(bookings) {
  return bookings.map(b => ({ card: b.Card, minute: b.Minute || null, player: b.IdPlayer || null }));
}

function parseSubstitutions(substitutions) {
  return substitutions.map(s => ({
    minute: s.Minute || null,
    playerOff: s.IdPlayerOff || null,
    playerOn: s.IdPlayerOn || null,
    reason: s.Reason ?? null,
  }));
}

function parseGoals(goals) {
  return goals.map(g => ({ type: g.Type, minute: g.Minute || null, player: g.IdPlayer || null, assist: g.IdAssistPlayer || null }));
}
