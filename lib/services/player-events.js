...
//
// Card detection is ANCHORED on the word "card". ESPN goal events carry
// type.text values like "Penalty - Scored" / "Own Goal - Scored" whose lowercase
// form contains the substrings "red" (sco-RED) and would otherwise be caught by
// a bare /red/ test — that mis-tagged Oyarzabal's converted penalty as a straight
// red and fabricated a suspension. Matching /red card/ / /yellow card/ only fires
// on genuine bookings, and never on scored penalties, shootout kicks, or own goals.
function classifyPlayerEventType(typeText, text) {
  ...
  // Own goals are not credited to the scorer, and penalty conversions
  // ("Penalty - Scored", including shootout kicks) are not tracked as goals here
  // — only open-play "Goal" events feed the star-form signal (KO-11).
  if (/own goal/.test(t)) return null;
