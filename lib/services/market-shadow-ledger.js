...
class MarketShadowLedger {
   ...
   */
  static recordSnapshot({ matchKey, kickoffTime, bookmaker, odds, rawResponse, dataDir }) {
    if (!matchKey || !odds) {
      throw new Error('matchKey and odds are required to record snapshot');
    }

    const ts = new Date().toISOString();
    const asOfValid = kickoffTime ? MarketShadowLedger.verifyAsOfAntiLeakage(ts, kickoffTime) : true;
  ...
  static compareModelVsMarket({ matchId, kickoffTime, modelPred, marketOdds, actualOutcome }) {
    ...
    const asOfValid = kickoffTime && marketOdds.ts
      ? MarketShadowLedger.verifyAsOfAntiLeakage(marketOdds.ts, kickoffTime)
      : true;

    let metrics = null;
    if (actualOutcome && ['home', 'draw', 'away'].includes(actualOutcome)) {
  ...
  static generateShadowBenchmarkReport(records = []) {
    ...
    return {
      ...
      coverage: {
        ...
        coveredMatches,
        missingMatches,
        coverageRate: totalMatches > 0 ? Number((coveredMatches / totalMatches).toFixed(4)) : 0
      },
      antiLeakageAudit: {
        asOfVerifiedMatches: asOfPassed,
        asOfFailedMatches: asOfFailed,
        leakageFreeRate: coveredMatches > 0 ? Number((asOfPassed / coveredMatches).toFixed(4)) : 1.0
      },
