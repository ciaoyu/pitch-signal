...
const { buildRefereeSection } = require('./referee-stats');
const { detectKnockout } = require('../knockoutStage');
const { buildStyleMatchupSection } = require('./matchup-styles');
const { buildStarFormSection } = require('./star-form');
const { buildLessonsSection } = require('./lessons-section');
const { buildGameStateSection } = require('./game-state-section');
const { buildSuperSubsSection } = require('./super-subs-section');
const { buildPenaltySection } = require('./penalty-section');
const { buildExperienceSection } = require('./experience-section');
const { guardKnockoutIntel } = require('./knockout-leakage-guard');
...
const SECTION_BUILDERS = [
  ...
  { key: 'styleMatchup',  build: (ctx) => buildStyleMatchupSection(ctx) },  // KO-12
  // { key: 'familiarity',   build: (ctx) => buildFamiliaritySection(ctx) },   // KO-?
  { key: 'gameState',     build: (ctx) => buildGameStateSection(ctx) },     // KO-13
  { key: 'experience',    build: (ctx) => buildExperienceSection(ctx) },    // KO-14
  { key: 'lessons',       build: (ctx) => buildLessonsSection(ctx) },       // KO-6
];
...
function normalizeCtx(ctx = {}) {
  ...
  return {
    ...
    db: ctx.db || null,
    // KO-15: preserve the prediction-time boundary for the final guard.
    kickoffTime: ctx.kickoffTime || ctx.matchDate || ctx.date || null,
    matchDate: ctx.matchDate || ctx.date || null,
    strictLeakageCheck: ctx.strictLeakageCheck === true,
  };
}
...
function buildKnockoutIntel(ctx = {}) {
  ...
  return {
    ...
    sections: guardKnockoutIntel(params, sections),
  };
}
