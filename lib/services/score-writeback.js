...
function writebackMatchScore(opts) {
  ...
  if (!rows || rows.length === 0) {
    ...
    const insertStmt = db.prepare(`
      INSERT INTO matches (group_id, home_team_id, away_team_id, home_score, away_score, played, match_date, venue, stage, went_to_et, decided_by_pens)
      VALUES (NULL, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?)
    `);
    const info = insertStmt.run(homeId, awayId, numHomeScore, numAwayScore, matchDate || null, venue || null, writebackStage, wentToEt ? 1 : 0, decidedByPens ? 1 : 0);
    const newId = Number(info.lastInsertRowid);
    if (logger && logger.info) {
      logger.info(`[score-writeback] Upserted previously-missing match row id=${newId} (${homeId} ${numHomeScore}-${numAwayScore} ${awayId}), stage=${writebackStage}, wentToEt=${!!wentToEt}, decidedByPens=${!!decidedByPens}, source=${source}, espnId=${espnId || 'N/A'}`);
    }
    return {
      success: true,
      updated: true,
  ...
  if (targetRow.played === 1 && targetRow.home_score !== null && targetRow.away_score !== null) {
    // Even when the score is already correct, still record final-status flags
    // (ET / penalties) on re-runs — they are independent of the score guard.
    writeFinalStatusFlags(targetRow.id, wentToEt, decidedByPens, logger);
  ...
  const updateStmt = db.prepare(`
    UPDATE matches
    SET home_score = ?, away_score = ?, played = 1, went_to_et = ?, decided_by_pens = ?
    WHERE id = ?
  `);

  const tx = db.transaction(() => {
    updateStmt.run(dbHomeScore, dbAwayScore, wentToEt ? 1 : 0, decidedByPens ? 1 : 0, targetRow.id);
  });
  tx();

  if (logger && logger.info) {
    logger.info(`[score-writeback] Successfully wrote back final score for match id=${targetRow.id} (${targetRow.home_team_id} ${dbHomeScore}-${dbAwayScore} ${targetRow.away_team_id}), wentToEt=${!!wentToEt}, decidedByPens=${!!decidedByPens}, source=${source}, espnId=${espnId || 'N/A'}`);
  }
...
/**
 * Record final-status flags (extra time / decided by penalties) for a match id.
 * Set-true-only: a `false` input does NOT clear an already-set flag, so the
 * live writeback path (which omits ET info) can never clobber a value the
 * one-time backfill derived authoritatively from ESPN.
 */
function writeFinalStatusFlags(matchId, wentToEt, decidedByPens, logger) {
  try {
    if (wentToEt) db.prepare('UPDATE matches SET went_to_et = 1 WHERE id = ?').run(matchId);
    if (decidedByPens) db.prepare('UPDATE matches SET decided_by_pens = 1 WHERE id = ?').run(matchId);
  } catch (e) {
    if (logger && logger.warn) logger.warn(`[score-writeback] ET/Pens flag update failed for match ${matchId}: ${e.message}`);
  }
}
