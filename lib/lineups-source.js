...
function loadBridge() {
  if (!_bridge) {
    try {
      _bridge = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'match_id_bridge.json'), 'utf8'));
...
function loadLineups() {
  if (!_lineups) {
    try {
      _lineups = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'lineups.json'), 'utf8'));
    } catch {
      _lineups = {};
    }
  }
...
function loadMatches() {
  if (!_matches) {
    try {
      _matches = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'matches.json'), 'utf8'));
    } catch {
      _matches = { matches: [] };
    }
  }
...
function loadSchedule() {
  if (!_schedule) {
    try {
      _schedule = JSON.parse(fs.readFileSync(path.join(ROOT_DATA, 'match_snapshot_schedule.json'), 'utf8'));
    } catch {
      _schedule = { matches: [] };
    }
  }
...
function loadRatings() {
  if (!_ratings) {
    try {
      _ratings = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'player-ratings.json'), 'utf8'));
    } catch {
      _ratings = { data: {} };
    }
  }
...
function getEspnTeamId(fifaCode) {
  try {
    const idBridge = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'id_bridge.json'), 'utf8'));
...
function getSubstitutions(espnMatchId) {
  ...
  const buildPlayerMap = (players) => {
    for (const p of players) {
      if (p.id && p.name) playerNames.set(String(p.id), p.name);
    }
  };
