...
class MatchReviewEngine {
  constructor(options = {}) {
    ...
    this.biasFactors = {
      ...
      lineup_changes: '首发调整/主力缺阵', // Star players benched or absent from starting XI
    };
  }
  ...
  getTeamRating(teamId) {
    // ratings.json is keyed by official team name (e.g. "Norway"), not the raw
    // ESPN numeric id passed in from the summary payload — resolve through the
    // shared team_resolver first (same path PredictionService uses) so this
    // doesn't silently fall back to the generic default for every match.
    const resolved = teamResolver.resolve(String(teamId || ''));
    const ratingsId = resolved?.ratings_id;
    return (
      (ratingsId && this.ratings[ratingsId]) ||
      this.ratings[teamId] ||
      { rating: 1500, attack_strength: 1.0, defense_strength: 1.0 }
    );
  }

  /**
   * Check lineup adjustments and key player absences
   */
  checkLineupChanges(matchData) {
    if (!matchData) return [];
    const evidence = [];
    const { homeId, awayId, lineups, roster, injuries, squadChanges } = matchData;
    ...
    // 1. Check direct injury/suspension list if provided in matchData
    const injuryList = injuries || squadChanges?.injuries || [];
    if (Array.isArray(injuryList)) {
      injuryList.forEach((item) => {
        if (!item) return;
        evidence.push({
          team: item.teamId === homeId ? homeName : (item.teamId === awayId ? awayName : item.team || 'Unknown'),
          player: item.player || item.name || '核心球员',
          type: 'absence_injury',
          detail: `${item.player || item.name} 因伤/停赛缺阵（${item.issue || '不可出战'}）`,
          detailI18n: this.i18n(`${item.player || item.name} 因伤/停赛缺阵（${item.issue || '不可出战'}）`, `${item.player || item.name} absent due to injury/suspension (${item.issue || 'unavailable'})`),
          impact: 'high',
        });
      });
    }

    // 2. Check lineups if available
    if (lineups && typeof lineups === 'object') {
      ['home', 'away'].forEach((side) => {
        const sideLineup = lineups[side];
        const teamName = side === 'home' ? homeName : awayName;
        if (!sideLineup) return;
        const bench = Array.isArray(sideLineup.bench) ? sideLineup.bench : Array.isArray(sideLineup.substitutes) ? sideLineup.substitutes : [];
        bench.forEach((p) => {
          const playerName = typeof p === 'string' ? p : p?.name || p?.player;
          const isStar = p?.isStar || p?.keyPlayer || (p?.marketValue && p.marketValue > 50000000);
          if (isStar && playerName) {
            evidence.push({
              team: teamName,
              player: playerName,
              type: 'star_on_bench',
              detail: `核心球员 ${playerName} 未进入首发名单，仅列为替补`,
              detailI18n: this.i18n(`核心球员 ${playerName} 未进入首发名单，仅列为替补`, `Key player ${playerName} started on the bench`),
              impact: 'high',
            });
          }
        });
    ...
    // 3. Fallback / supplementary check via top 3 high market value players from market-values.json
    try {
      const mvPath = path.join(__dirname, '..', 'data', 'market-values.json');
      if (fs.existsSync(mvPath)) {
        const mvData = JSON.parse(fs.readFileSync(mvPath, 'utf8'));
        const teams = mvData.teams || mvData.nationalTeams || [];
        [ { id: homeId, name: homeName, en: homeNameEn }, { id: awayId, name: awayName, en: awayNameEn } ].forEach(({ id, name, en }) => {
          const teamEntry = Array.isArray(teams) ? teams.find(t => String(t.id) === String(id) || t.name === en) : null;
          if (teamEntry && Array.isArray(teamEntry.players)) {
            const topPlayers = [...teamEntry.players]
              .sort((a, b) => (b.marketValueEur || b.value || 0) - (a.marketValueEur || a.value || 0))
              .slice(0, 3);
            topPlayers.forEach((p) => {
              const playerName = p.name || p.player;
              if (p.injured || p.suspended || p.absent) {
                evidence.push({
                  team: name,
                  player: playerName,
                  type: 'key_player_absent',
                  detail: `高身价主力 ${playerName} 缺席首发阵容`,
                  detailI18n: this.i18n(`高身价主力 ${playerName} 缺席首发阵容`, `High-value core player ${playerName} absent from starting XI`),
                  impact: 'high',
                });
              }
            });
  ...
  generateReview(matchData) {
    ...
    // ========== 5. Key Events & Lineup Evidence ==========
    const keyEvents = matchData.events || this.generateDefaultEvents(matchData);
    const lineupEvidence = this.checkLineupChanges(matchData);
  ...
  analyzeBias(matchData, homeRating, awayRating, prediction) {
    ...
    const lineupEvidence = this.checkLineupChanges(matchData);
    if (lineupEvidence && lineupEvidence.length > 0) {
      lineupEvidence.forEach((le) => {
        factors.push({
          key: `lineup_change_${le.player}`,
          factor: this.biasFactors.lineup_changes || '首发调整/主力缺阵',
          factorI18n: this.i18n(this.biasFactors.lineup_changes || '首发调整/主力缺阵', 'Lineup changes / Key absences'),
          detail: le.detail,
          detailI18n: le.detailI18n || this.i18n(le.detail, le.detail),
          impact: le.impact || 'high',
        });
      });
    }
