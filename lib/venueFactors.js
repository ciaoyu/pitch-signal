...
const WC2026_DIR = path.join(__dirname, '..', 'data', 'sources', 'seed', 'wc2026');
...
function loadData() {
  ...
  const readJson = (file) => {
    try {
      return JSON.parse(fs.readFileSync(path.join(WC2026_DIR, file), 'utf8'));
    } catch { console.debug('venueFactors: metadata fetch failed');
      return null;
    }
  };
