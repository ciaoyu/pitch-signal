...
function fetchSnapshotRows(database = db, options = {}) {
  const whereClauses = [
    'r.actual_home_score IS NOT NULL',
    'r.actual_away_score IS NOT NULL',
    's.home_win_prob IS NOT NULL',
    's.draw_prob IS NOT NULL',
    's.away_win_prob IS NOT NULL',
  ];
  const params = [];

  if (options.modelVersion) {
    if (options.modelVersion === 'legacy') {
      whereClauses.push('s.model_version IS NULL');
    } else {
      whereClauses.push('s.model_version = ?');
      params.push(String(options.modelVersion));
    }
  }
  if (options.configHash) {
    whereClauses.push('s.config_hash = ?');
    params.push(String(options.configHash));
  }
  if (options.verificationStatus) {
    whereClauses.push('s.verification_status = ?');
    params.push(String(options.verificationStatus));
  }
  ...
  return rows.map((row) => ({
    ...
    verificationStatus: row.verification_status || (row.model_version ? 'verified' : 'legacy'),
  })).filter((row) => (
...
function buildCalibrationReport(options = {}) {
  const rows = options.rows || fetchSnapshotRows(options.db || db, options);
  ...
  return {
    ...
    modelVersionFilter: options.modelVersion || null,
    configHashFilter: options.configHash || null,
    verificationStatusFilter: options.verificationStatus || null,
    sampleSize,
