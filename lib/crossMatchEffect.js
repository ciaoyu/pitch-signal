 ...
 * @returns {string|null}
 */
function extractTeamLessonPair(review, teamId) {
  ...
  for (const key of Object.keys(teamSpecific)) {
    if (aliases.has(key)) {
      ...
      if (typeof v === 'string' && v.trim()) {
        console.debug(`crossMatchEffect: legacy string lesson used for ${key}`);
        return { zh: v.trim(), en: null, legacySingleLanguage: 'zh' };
      }
      if (v && typeof v === 'object') {
        const zh = typeof v.zh === 'string' ? v.zh.trim() : '';
        const en = typeof v.en === 'string' ? v.en.trim() : '';
        if (zh || en) return { zh: zh || en, en: en || zh, legacySingleLanguage: !en ? 'zh' : (!zh ? 'en' : null) };
      }
    }
  }
  return null;
...
function extractTeamLesson(review, teamId) {
  const pair = extractTeamLessonPair(review, teamId);
  return pair ? (pair.en || pair.zh) : null;
}
...
function extractGlobalInsights(review) {
  ...
  if (typeof globalModel === 'string' && globalModel.trim()) result.globalModel = globalModel.trim();
  else if (globalModel && typeof globalModel === 'object') {
    result.globalModel = String(globalModel.en || globalModel.zh || '').trim() || null;
  }
...
analyzer.extractTeamLessonPair = extractTeamLessonPair;
