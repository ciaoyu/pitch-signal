...
class EloRating {
  ...
  /**
   * 计算预期胜率
   * @param {number} ratingA - 队伍 A 评分
   * @param {number} ratingB - 队伍 B 评分
   * @returns {number} 队伍 A 的预期胜率 (0-1)
   */
   ...
   * 比分差加成系数
   * 大比分胜利获得更多评分变动
   * @param {number} goalDiff - 进球差（正数=主队赢）
   * @param {number} eloDiff - 评分差
   * @returns {number} 加成系数 (>= 1)
   */
  ...
  /**
   * 更新比赛双方评分
   * @param {number} homeRating - 主队当前评分
   * @param {number} awayRating - 客队当前评分
   * @param {number} homeScore - 主队进球数
   * @param {number} awayScore - 客队进球数
   * @param {object} options - 可选参数 { kFactor }
   * @returns {object} 新评分和详情
   */
  updateRatings(homeRating, awayRating, homeScore, awayScore, options = {}) {
    ...
    // 预期胜率（含主场优势）
    const expectedHome = this.expectedScore(homeRating + this.homeAdvantage, awayRating);
  ...
  /**
   * 基于 FIFA 排名初始化 Elo 评分
   * 线性映射：rank 1 → 2100, rank 50 → 1500
   * @param {number} rank - FIFA 排名
  ...
  /**
   * 预测比赛结果概率
   * @param {number} homeRating - 主队 Elo 评分
   * @param {number} awayRating - 客队 Elo 评分
   * @returns {object} 胜平负概率
   */
  predictMatch(homeRating, awayRating) {
    const expectedHome = this.expectedScore(homeRating + this.homeAdvantage, awayRating);
   ...
   * 估算平局概率
   * 评分差越小，平局概率越高
   * @param {number} homeRating - 主队评分
   * @param {number} awayRating - 客队评分
   * @returns {number} 平局概率 (0.15 ~ 0.30)
   */
