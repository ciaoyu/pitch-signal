/**
 * 共享的积分榜计算逻辑
 * 用于 core.js 和 standings.js 避免重复代码
 */
...
async function getCompletedGroupMatches(espn, parseEvent, daysBack = 14) {
  const matches = [];
  const now = Date.now();
  for (let i = -daysBack; i <= 0; i++) {
    ...
    } catch { console.debug('standings-helper: JSON parse of fixture payload failed'); }
  }
  return matches;
}
...
function buildGroupsFromStandings(standingsMap, getTeamNameZh, getTeamNameI18n, TEAM_LOGOS, espnMap = null) {
  ...
  for (const [groupName, teamIds] of Object.entries(WC_GROUPS)) {
    const standings = teamIds
      .map((id) => {
        ...
        if (espnMap && espnMap[id]) {
          ...
          return computed.played > espnTeam.played ? { ...espnTeam, ...computed } : espnTeam;
        }
        return computed;
      })
      .sort((a, b) => {
    ...
    groups.push({ name: `小组 ${groupName}`, standings });
  }
  return groups;
}
