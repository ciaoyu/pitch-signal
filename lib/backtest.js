...
function loadAllHistoryMatches() {
  const dir = path.join(__dirname, '..', 'data', 'history');
  const byYear = {};
  let files = [];
  try { files = fs.readdirSync(dir); } catch (_) { files = []; }
  for (const f of files) {
    const match = f.match(/^worldcup_(\d{4})\.json$/);
    if (!match) continue;
    try {
      const rows = JSON.parse(fs.readFileSync(path.join(dir, f), 'utf8')).matches || [];
      byYear[match[1]] = rows.map(row => BacktestRunner.annotateMatchVenue(row, match[1]));
    } catch (error) {
      console.error(`Failed to load ${f}:`, error.message);
    }
  }
  const all = Object.keys(byYear).sort().flatMap(year => byYear[year]);
  return { m2018: byYear['2018'] || [], m2022: byYear['2022'] || [], byYear, all };
}
...
module.exports.loadAllHistoryMatches = loadAllHistoryMatches;
