  ...
  async _walkForward(matches, paramOverrides = null, evalFilter = null, opts = {}) {
    ...
    for (const dateStr of sortedDates) {
      ...
      for (const match of dayMatches) {
        ...
        // P0 quarantine (Owner A v2): every World Cup match is played on a
        // neutral pitch, so the nominal home advantage is CLOSED for all of
        // them (host effect size waits for data estimation — Owner E). The host
        // nation is recorded as a FACT only (hostSide) and is never a probability
        // bonus. Matches are pre-annotated by annotateMatchVenue (isHostHome /
        // isHostAway) earlier in the walk-forward loop.
        const HOST_COUNTRY_TEAM_ID = { USA: 'USA', 'United States': 'USA', Canada: 'Canada', Mexico: 'Mexico' };
        let hostTeamId = null;
        if (match.isHostHome) hostTeamId = HOST_COUNTRY_TEAM_ID[match.home] || match.home;
        else if (match.isHostAway) hostTeamId = HOST_COUNTRY_TEAM_ID[match.away] || match.away;
