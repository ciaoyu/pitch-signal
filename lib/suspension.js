...
function evaluatePlayerSuspension(events, opts = {}) {
  ...
  for (const evs of byMatchAll.values()) {
    ...
    if (yellows >= 2 && !hasRed) secondYellowMatch = true;
  }

  // Straight/second-yellow reds (never wiped by the yellow reset).
  ...
  if (reds.length > 0) {
    suspended = true;
    reason = reds.some((e) => e.event_type === 'red') ? 'red' : 'second_yellow';
    pendingDisciplinary = reds.some((e) => e.event_type === 'red'); // straight red => committee review
  } else if (secondYellowMatch) {
    suspended = true;
