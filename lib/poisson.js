...
class PoissonModel {
  ...
  /**
   * 当 xgProfile 有足够数据时（≥2场），用真实 xG 校准 λ。
   * 混合比例：λ_math 60% + λ_xg 40%（数据充足时）
   * 没有 xG 数据时透明降级，与原逻辑完全一致。
   */
  blendXg(lambdaMath, xgProfile, isHome) {
    if (!xgProfile || xgProfile.matches < 2) return lambdaMath;
    const globalXg = this.globalAvgGoals; // 1.2
    const teamXg = isHome ? xgProfile.avgXgFor : xgProfile.avgXgFor;
