...
db.exec(`
  ...
  -- Player-level match events (KO-4): single source of truth for suspension
  -- ledger (KO-4) and star-form signal (KO-11). One table, two consumers.
  -- Generic event table; idempotent inserts via UNIQUE(match_id, player_name,
  -- event_type, minute, minute_added) so backfill can be re-run safely.
  CREATE TABLE IF NOT EXISTS player_match_events (
    ...
    player_id     TEXT,
    event_type    TEXT    NOT NULL CHECK (event_type IN ('goal','assist','yellow','red','secondyellow')),
    minute        INTEGER NOT NULL DEFAULT 0,
    minute_added  INTEGER NOT NULL DEFAULT 0,
    stage         TEXT,
    round         TEXT,
  ...
  CREATE INDEX IF NOT EXISTS idx_player_events_match ON player_match_events(match_id);
  CREATE INDEX IF NOT EXISTS idx_player_events_player ON player_match_events(player_name);

  -- Match officials (KO-10): referee assignment and historical stats
  CREATE TABLE IF NOT EXISTS match_officials (
    ...
    raw_json      TEXT,
    created_at    TEXT DEFAULT (datetime('now')),
    UNIQUE(match_id, name, role)
  );
  CREATE INDEX IF NOT EXISTS idx_match_officials_match ON match_officials(match_id);
  CREATE INDEX IF NOT EXISTS idx_match_officials_name ON match_officials(name);
...
(function runMigrations() {
  const migrations = [
    ...
    `ALTER TABLE matches ADD COLUMN went_to_et INTEGER DEFAULT 0`,
    `ALTER TABLE matches ADD COLUMN decided_by_pens INTEGER DEFAULT 0`,
    ...
    `ALTER TABLE prediction_snapshots ADD COLUMN request_path TEXT`,
    `ALTER TABLE prediction_snapshots ADD COLUMN verification_status TEXT DEFAULT 'legacy'`,
    `UPDATE prediction_snapshots SET verification_status = 'legacy' WHERE (model_version IS NULL OR config_hash IS NULL) AND (verification_status IS NULL OR verification_status = 'verified')`,
  ];
  for (const sql of migrations) {
    try { db.exec(sql); } catch (_) { /* Column already exists or error, ignore */ }
  }
})();
