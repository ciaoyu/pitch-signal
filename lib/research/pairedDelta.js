...
class PairedDeltaEvaluator {
  ...
  static computeClusteredPairedDelta(records, options = {}) {
    ...
    const formatPValueDisplay = (p, extremeCount, totalResamples) => {
      if (p < 0.001) return `<0.001 (${extremeCount}/${totalResamples})`;
      return p.toFixed(3);
    };
    ...
    return {
      ...
      brier: {
        ...
        pValueDisplay: formatPValueDisplay(
          pValueTwoSidedNullCentered,
          nullExtremeBrierCount,
          nResamples
        ),
