...
class ArtifactGenerator {
  ...
  /**
   * Check if a prospective record satisfies strict verified immutable ledger criteria:
   * - probability validity: finite numbers in [0,1], homeWin + draw + awayWin close to 1
   * - verificationStatus === 'verified'
   * - modelVersion === 'p0-quarantine-v3-2026-07-10'
   * - configHash === '2066763607e5' (A v4)
   * - predictedAt strictly earlier than kickoffTime
   * - source/track matches public Track A / live-reprice
   * - actualOutcome belongs to post-match factual results ('home'|'draw'|'away')
   */
  static isVerifiedRecord(r) {
    if (!r || typeof r !== 'object' || !r.pred || typeof r.pred !== 'object') return false;
    if (!['home', 'draw', 'away'].includes(r.actualOutcome)) return false;

    // P1 Item 1: Strict probability check (finite, [0,1], sum close to 1)
    const { homeWin, draw, awayWin } = r.pred;
    if (!Number.isFinite(homeWin) || !Number.isFinite(draw) || !Number.isFinite(awayWin)) return false;
    if (homeWin < 0 || homeWin > 1 || draw < 0 || draw > 1 || awayWin < 0 || awayWin > 1) return false;
    if (Math.abs(homeWin + draw + awayWin - 1.0) > 1e-3) return false;

    const vStatus = r.verificationStatus || r.verification_status;
    if (vStatus !== 'verified') return false;

    const modelVer = r.modelVersion || r.pred.modelVersion;
    if (modelVer !== 'p0-quarantine-v3-2026-07-10') return false;

    const cfgHash = r.configHash || r.pred.configHash;
    if (cfgHash !== '2066763607e5') return false;

    const predAt = r.predictedAt || r.pred.predictedAt;
    const kickoff = r.kickoffTime || r.matchDate || r.date;
    if (!predAt || !kickoff) return false;
    const predTs = new Date(predAt).getTime();
    const kickTs = new Date(kickoff).getTime();
    if (isNaN(predTs) || isNaN(kickTs) || predTs >= kickTs) return false;

    const src = r.source || r.pred.source;
    const trk = r.track || r.pred.track;
    const validSrc = ['Track A', 'live-reprice', 'elo_poisson'].includes(src) || ['Track A', 'public'].includes(trk);
   ...
   * Never hardcodes research evidence metrics. Enforces strict verified immutable ledger checks.
   * Outputs comprehensive auditCounts breakdown (total / completed / eligible / rejected / pending).
   */
  static computeProspectiveMetrics(options = {}) {
    ...
    if (!fs.existsSync(ledgerPath)) {
      return {
        ...
        auditCounts: { total: 0, completed: 0, eligible: 0, rejected: 0, pending: 0 },
        metrics: {
          meanBrier: null,
          topLabelECE: null,
    ...
    try {
      ...
      const completedRecords = records.filter(r => r && r.pred && ['home', 'draw', 'away'].includes(r.actualOutcome));
      const pendingCount = total - completedRecords.length;
      const eligibleRecords = completedRecords.filter(r => ArtifactGenerator.isVerifiedRecord(r));
      const eligibleCount = eligibleRecords.length;
      const rejectedCount = completedRecords.length - eligibleCount;
      ...
      if (eligibleCount === 0 || rejectedCount > 0) {
        return {
          status: 'unverified',
          sampleSize: null,
          description: 'Prospective online live service predictions for 2026 matches (awaiting strictly verified immutable ledger)',
          auditCounts,
          metrics: {
            meanBrier: null,
            topLabelECE: null,
          ...
          note: rejectedCount > 0
            ? `Ledger export contains ${rejectedCount} rejected completed records failing probability normalization or strict immutable pre-match criteria; marked unverified/null.`
            : `Ledger export contains 0 eligible completed matches (${pendingCount} pending); marked unverified/null.`
        };
      }

      const validRecords = completedRecords;
  ...
  static async writeArtifacts(outputDir, backtestResults, commandStr = 'node scripts/research-generate-artifacts.js', options = {}) {
    ...
    const counts = prospectiveMetrics.auditCounts || { total: 0, completed: 0, eligible: 0, rejected: 0, pending: 0 };
    const auditStr = `Audit Breakdown: Total=${counts.total}, Completed=${counts.completed}, Eligible=${counts.eligible}, Rejected=${counts.rejected}, Pending=${counts.pending}`;
