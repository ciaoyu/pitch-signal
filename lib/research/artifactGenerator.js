...
class ArtifactGenerator {
   ...
   * If ledger is missing/empty or contains mixed/legacy/unverified records, returns unverified/null.
   */
  static computeProspectiveMetrics(options = {}) {
    ...
    try {
      ...
      const completedRecords = records.filter(r => r && r.pred && r.actualOutcome);
      if (completedRecords.length === 0) {
        return {
          ...
          inputHash,
          note: 'Ledger file found but contains 0 completed matches; marked unverified/null.'
        };
      }

      // Check if any completed record fails strict verification or is a mixed/legacy model version
      const allVerified = completedRecords.every(r => ArtifactGenerator.isVerifiedRecord(r));
      if (!allVerified) {
        return {
          status: 'unverified',
          sampleSize: null,
          ...
          inputFile: path.basename(ledgerPath),
          inputHash,
          note: 'Ledger export contains unverified, legacy, or mixed model version records failing strict criteria; prospective evaluation rejected and marked unverified/null.'
        };
      }
