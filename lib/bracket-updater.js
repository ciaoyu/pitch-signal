...
function loadMatchIdBridge() {
  if (!_matchIdBridge) {
    try {
      _matchIdBridge = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'sources', 'seed', 'wc2026', 'match_id_bridge.json'), 'utf8'));
    } catch (e) {
...
function propagateResults(bracket, deps) {
  ...
  for (const [slotId, m] of Object.entries(bracket.matches)) {
    ...
    if (fifaMatch) {
      // 用 FIFA match ID 反查 ESPN ID 后写入 espnMatchId / matchId 字段
