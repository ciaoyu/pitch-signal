...
function buildResolvedBracket({ posMap, posMapI18n, thirdPlaceData, bracket, schedule, deps }) {
  ...
  for (const [id, m] of Object.entries(bracket.matches)) {
    ...
    // teamFromScheduleOrCode() resolves name/id from the raw ESPN schedule
    // (which carries no nameI18n), so it always sets nameI18n: null — that
    // Object.assign above then clobbers whatever resolveSlot() already had.
    // Backfill from the numeric ESPN id when available.
    if (!teamAResolved.nameI18n && teamAResolved.id && deps?.getTeamNameI18n) {
      teamAResolved.nameI18n = deps.getTeamNameI18n(teamAResolved.id);
    }
    if (!teamBResolved.nameI18n && teamBResolved.id && deps?.getTeamNameI18n) {
      teamBResolved.nameI18n = deps.getTeamNameI18n(teamBResolved.id);
    }
