...
function buildR32MatchMap(bracket, scheduleMatches) {
  ...
  for (const [slotId, m] of Object.entries(bracket.matches)) {
    ...
    const schedMatch = knockoutMatches.find(s => s.shortName === shortName);
    if (schedMatch) {
      map[slotId] = {
...
function propagateResults(bracket, deps) {
  // Phase 1: 淘汰赛未开始，不做任何修改
  // TODO Phase 2: 实现 ESPN 查询 + 结果传播
  //   - 读取 bracket_slot_map.json 的 espnMatchId
  //   - 查询 /scoreboard?dates=YYYYMMDD
  //   - 通过 team ID 匹配 R32，通过 espnMatchId 匹配 R16+
  //   - 更新 winner 并传播到下游 slots
  return bracket;
}
