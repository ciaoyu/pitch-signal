...
// Which side actually progressed in a knockout tie. For shootouts the goal
// scoreline is level (regulation + ET draw), so the shootout decides; for AET
// wins the final scoreline decides. Returns 'home' | 'away' | null (undetermined).
function knockoutAdvancer(match, finalHomeScore, finalAwayScore) {
  if (Boolean(match.decidedByPenalties)) {
    const sh = Number(match.shootoutHomeScore);
    const sa = Number(match.shootoutAwayScore);
    if (Number.isFinite(sh) && Number.isFinite(sa) && sh !== sa) return sh > sa ? 'home' : 'away';
    return null;
  }
  const result = actualResult(finalHomeScore, finalAwayScore);
...
// Factor for a knockout tie where regulation ended level but the model's leaned
// side went on to advance. Framed as a low-impact note (predicted the winner of
// the tie), not a direction miss, while making the 90-minute scope explicit.
function knockoutAdvancerFactor({ homeScore, awayScore, advancerName, advanceMethodZh, advanceMethodEn, retro }) {
  const zhLead = retro ? '赛后回溯模拟' : '赛前预测';
  const enLead = retro ? 'The retrospective simulation' : 'The pre-match forecast';
  const zh = `常规时间 ${homeScore}-${awayScore} 打平（预测域仅为 90 分钟胜平负），${zhLead}看好的 ${advancerName} 通过${advanceMethodZh}晋级，命中晋级方。`;
  const en = `Regulation ended level at ${homeScore}-${awayScore} (the forecast domain is the 90-minute result only); ${enLead.toLowerCase()} leaned toward ${advancerName}, who advanced via ${advanceMethodEn} — the advancer was called correctly.`;
  return {
    key: 'knockout_regulation_split',
    factor: '常规时间打平，命中晋级方',
    factorI18n: i18n('常规时间打平，命中晋级方', 'Regulation draw, advancer called correctly'),
    impact: 'low',
    detail: zh,
    detailI18n: i18n(zh, en),
  };
}
...
function teamIdsMatch(a, b, allowMissing = true) {
  const aa = a == null || a === '' ? null : String(a);
  const bb = b == null || b === '' ? null : String(b);
  if (aa === null || bb === null) return allowMissing ? true : false;
  return aa === bb;
}
...
function snapshotTeamsMatch(snapshot, match) {
  ...
  const isDbOrExplicitSnapshot = Boolean(snapshot.id || 'homeTeamId' in snapshot || 'awayTeamId' in snapshot);
  const allowMissing = !isDbOrExplicitSnapshot;
  return teamIdsMatch(snapshot.homeTeamId, match.homeId, allowMissing)
    && teamIdsMatch(snapshot.awayTeamId, match.awayId, allowMissing);
}
...
function getPredictionSnapshotForTeams(matchId, homeTeamId, awayTeamId) {
  const rows = db.prepare(`
    ...
    ORDER BY created_at DESC
  `).all(String(matchId));
  const targetHome = homeTeamId == null || homeTeamId === '' ? null : String(homeTeamId);
  const targetAway = awayTeamId == null || awayTeamId === '' ? null : String(awayTeamId);
  for (const row of rows) {
    const rowHome = row.home_team_id == null || row.home_team_id === '' ? null : String(row.home_team_id);
    const rowAway = row.away_team_id == null || row.away_team_id === '' ? null : String(row.away_team_id);
    if (targetHome && targetAway) {
      if (rowHome && rowAway && rowHome === targetHome && rowAway === targetAway) {
        return normalizeSnapshot(row);
      }
    } else if (teamIdsMatch(row.home_team_id, homeTeamId) && teamIdsMatch(row.away_team_id, awayTeamId)) {
      return normalizeSnapshot(row);
    }
  }
  return null;
...
function getMatchDayReportNote(homeName, awayName) {
  ...
  // multi-match summary sections (e.g. "Match Day Review: ...") mention all teams from that day in the same section;
  ...
  // starting with |) and ignore prose; single-match "Post-Match Review" subsections have uniquely-identifying titles
...
// Sections starting with "Tactical Patterns: ..." are general signal patterns distilled from multiple matches
...
function buildPostMatchReview(input) {
  ...
  // Knockout ties settled in extra time or on penalties. The public 1X2 output
  // is a 90-minute (regulation) forecast, so a regulation draw legitimately
  // makes resultCorrect=false for calibration/Brier purposes. But when the
  // model leaned to a side and *that side advanced*, it picked the winner of the
  // tie — do not brand that a "wrong result / forecast miss" to the user.
  const isKnockout = wentToExtraTime || decidedByPenalties;
  const advancer = isKnockout ? knockoutAdvancer(match, finalHomeScore, finalAwayScore) : null;
  const predictedSide = predictedResult === 'home' || predictedResult === 'away' ? predictedResult : null;
  const advancerCorrect = isKnockout && actual === 'draw' && !resultCorrect
    && predictedSide != null && advancer != null && predictedSide === advancer;
  const advancerName = advancer === 'home' ? homeName : advancer === 'away' ? awayName : null;
  const advanceMethodZh = decidedByPenalties ? '点球大战' : '加时赛';
  const advanceMethodEn = decidedByPenalties ? 'a penalty shootout' : 'extra time';
  ...
  if (!snapshot) {
    ...
    if (advancerCorrect) {
      factors.push(knockoutAdvancerFactor({ homeScore, awayScore, advancerName, advanceMethodZh, advanceMethodEn, retro: true }));
  ...
  } else if (advancerCorrect) {
    factors.push(knockoutAdvancerFactor({ homeScore, awayScore, advancerName, advanceMethodZh, advanceMethodEn, retro: false }));
  } else if (!resultCorrect) {
    const predictedLabel = resultLabelI18n(predictedResult, homeName, awayName);
    const actualLabel = resultLabelI18n(actual, homeName, awayName);
    factors.push({
  ...
  const accuracy = scoreExact
    ? 'exact_score'
    : resultCorrect
      ? 'result_correct_score_wrong'
      : advancerCorrect
        ? 'regulation_draw_advancer_correct'
        : 'wrong_result';
  const advancerSummaryZh = `${isRetro ? '赛后回溯模拟' : '赛前预测'}的 90 分钟胜平负因常规时间打平而未命中，但模型看好的 ${advancerName} 通过${advanceMethodZh}晋级——命中晋级方。请注意：公开预测域仅为常规时间。`;
  const advancerSummaryEn = `${isRetro ? "The retrospective simulation's" : "The pre-match forecast's"} 90-minute result missed because regulation ended level, but the model's leaned side ${advancerName} advanced via ${advanceMethodEn} — the advancer was called correctly. Note: the public forecast domain is regulation time only.`;
  ...
  const summaryI18n = scoreExact
    ...
    : resultCorrect
      ...
      : advancerCorrect
        ? i18n(summary, advancerSummaryEn)
  ...
  const review = {
    ...
    matchSummary: {
      matchTypeKey: scoreExact ? 'exact_score' : resultCorrect ? 'direction_hit' : advancerCorrect ? 'knockout_regulation_split' : 'forecast_miss',
    ...
    biasAnalysis: {
      ...
      actualResult: actual,
      resultCorrect,
      // Knockout advancer dimension: kept separate from resultCorrect so the
      // 90-minute 1X2 calibration truth is preserved for Brier scoring.
      advancer,
      advancerCorrect,
      scoreExact,
      accuracy,
