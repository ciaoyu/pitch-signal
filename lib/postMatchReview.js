...
function summarizeSnapshotNode(node, match) {
  ...
  const detailZh = node.summary || node.analysis || '';
  return {
    minute,
    ...
    summaryI18n: {
      ...
      en: [baseZh, detailZh, oddsTextZh].filter(Boolean).join(' · '),
    },
  };
}
...
function normalizeSnapshot(row) {
  ...
  return {
    ...
    venueSemantics: safeJsonParse(row.venue_semantics_json, {
    ...
    verificationStatus: isVerified ? (row.verification_status || 'verified') : 'legacy',
    payload,
  };
}
