/**
 * Bot Knowledge Base — PitchSignal
 * 
 * Loads docs/*.md at startup, builds keyword index by ## sections.
 * Provides search(query, lang) returning matching markdown sections.
 */
const fs = require('fs');
...
// Manually curated keyword map per ## section (section title → natural language query keywords).
// Sections not in this map are auto-indexed by title tokenization.
...
// Inverted index: keyword → set of section ids
...
function tokenize(text) {
  ...
  // Extract CJK characters + English words
  const tokens = [];
  // English words
  ...
  // Short English words also indexed separately
 ...
 * @returns {{ sections: Array, isExact: boolean }} matched section list
 */
function search(query, lang = 'zh') {
  ...
  // Score by matched keyword count
  ...
  // Also do title fuzzy match (user may use similar but not exact words)
  ...
  if (scored.size === 0) {
    // Empty key fallback: return default section
    ...
    // Otherwise return first few core sections
...
/**
 * Assemble search results into a long text block for use as system prompt context.
 */
