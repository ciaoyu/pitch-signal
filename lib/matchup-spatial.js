...
module.exports = function initMatchupSpatial(deps) {
  ...
  function buildSpatialMatchup(homeId, awayId, ratingsData, options = {}) {
    ...
    const toPlayer = (p) => {
      // Determine player ID (slug or ESPN athlete ID)
      const playerId = p.id || p.fifaPlayerId || null;
      
      // Resolve photo URL
      let photo = null;
      if (playerId) {
        // If ESPN athlete ID (numeric string), fetch photo directly
        if (/^\d{6,7}$/.test(String(playerId))) {
          photo = playerIdResolver.getPhotoByEspnId(playerId);
        } else {
          // If slug, resolve via resolver
    ...
    return {
      ...
      // T3 integration: formation/XI/substitutions data source
