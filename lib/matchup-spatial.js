...
module.exports = function initMatchupSpatial(deps) {
  ...
  function buildSpatialMatchup(homeId, awayId, ratingsData) {
    ...
    return {
      ...
      home: { name: getTeamNameZh(homeLookup.espnId), flag: homeLookup.resolved?.flag || TEAM_FLAGS[homeLookup.ratingsId] || TEAM_FLAGS[homeLookup.espnId] || '🏳️', formation: homeFormation, players: [...homeCoords.gk, ...homeCoords.def, ...homeCoords.mid, ...homeCoords.fwd].map(toPlayer) },
      away: { name: getTeamNameZh(awayLookup.espnId), flag: awayLookup.resolved?.flag || TEAM_FLAGS[awayLookup.ratingsId] || TEAM_FLAGS[awayLookup.espnId] || '🏳️', formation: awayFormation, players: [...awayCoords.gk, ...awayCoords.def, ...awayCoords.mid, ...awayCoords.fwd].map(toPlayer) },
      pairs,
