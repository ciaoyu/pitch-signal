...
module.exports = function initMatchupSpatial(deps) {
  ...
  function resolveRealLineup(lineupInput, squadPlayersArray, formation, isHome) {
    const resolvedPlayers = lineupInput.map(p => {
      ...
      return { ...p, rating, name: matchedName, jersey: matchedJersey };
    });
  ...
  function pairPlayers(home, away) {
    ...
    function makePair(h, a, zone) {
      ...
      return {
        home: { name: h.name, x: h.x, y: h.y, rating: h.rating, playerId: h.playerId || null, photo: h.photo || null },
        away: { name: a.name, x: a.x, y: a.y, rating: a.rating, playerId: a.playerId || null, photo: a.photo || null },
        diff: gap, zone,
