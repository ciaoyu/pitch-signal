...
const POSTMORTEM_MODEL = process.env.CLAUDE_POSTMORTEM_MODEL || 'claude-sonnet-4-6';
// 用户问答:高频、低成本
const CHAT_MODEL = process.env.CLAUDE_CHAT_MODEL || 'claude-haiku-4-5';

// 复盘专用 provider(可选):配置 POSTMORTEM_API_KEY 后,赛后复盘优先用它,
// 调用失败时自动回落到默认客户端(通常是 DeepSeek)。bot 问答不受影响。
const POSTMORTEM_API_KEY = process.env.POSTMORTEM_API_KEY || '';
const POSTMORTEM_BASE_URL = process.env.POSTMORTEM_BASE_URL || '';
const POSTMORTEM_PRIMARY_MODEL = process.env.POSTMORTEM_MODEL || POSTMORTEM_MODEL;

let _client = null;
let _postmortemClient = null;
 ...
 *               CLAUDE_CHAT_MODEL=deepseek-v4-flash
 *
...
function getClient() {
  ...
  _client = buildClient(apiKey, process.env.ANTHROPIC_BASE_URL);
  return _client;
}

/**
 * 复盘专用客户端。未配置 POSTMORTEM_API_KEY 时返回 null,
 * 由 analyzePostMatch 直接使用默认客户端。
 */
function getPostmortemClient() {
  if (_postmortemClient) return _postmortemClient;
  if (!POSTMORTEM_API_KEY) return null;
  _postmortemClient = buildClient(POSTMORTEM_API_KEY, POSTMORTEM_BASE_URL);
  return _postmortemClient;
}
...
async function analyzePostMatch(aiPromptContext) {
  ...
  // 1) 复盘专用 provider 优先(若已配置)
  const primary = getPostmortemClient();
  if (primary) {
    try {
      return await runPostmortem(primary, POSTMORTEM_PRIMARY_MODEL, POSTMORTEM_BASE_URL, prompt);
    } catch (e) {
      console.log('[postmortem] 专用 provider 失败,回落到默认(DeepSeek):', e.message);
    }
  }

  // 2) 回落:默认客户端(通常是 DeepSeek)
...
async function chat(messages, systemPrompt) {
  ...
  const res = await client.messages.create({
    ...
    ...deepSeekThinkingOptions(false, process.env.ANTHROPIC_BASE_URL),
    messages,
  });
