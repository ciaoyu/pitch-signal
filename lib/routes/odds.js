  ...
  'GET /api/odds/:matchId': async (params) => {
    ...
    if (ODDS_API_KEY) {
      try {
        ...
        // Not all APIs support headers the same way, but most proxy auth keys to x-api-key or apiKey header.
        // Odds API supports apiKey parameter in query, but if forced to use header, often x-api-key or Authorization works.
        // Actually, The Odds API documentation says: "The API Key can be passed in the apiKey query parameter" but we will just omit from URL and pass via header if requested.
    ...
    if (matched) {
      ...
      const result = {
        ...
        corners: { line: 9.5, over: null, under: null },
