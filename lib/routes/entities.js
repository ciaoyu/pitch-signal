...
function buildEstimatedTeamOverview(teamRating, standings, teamId, teamData = {}, eloRankMap = {}) {
  ...
  let teamMeta = {};
  try { teamMeta = require(require('path').join(process.cwd(), 'data', 'team_meta.json')); } catch(e) {}
...
module.exports = function createEntityRoutes(deps) {
  ...
  return {
    ...
    'GET /api/team/:id/enhanced': async (params) => {
      ...
      let teamMetaAll = {};
      try { teamMetaAll = require(require('path').join(process.cwd(), 'data', 'team_meta.json')); } catch(e) {}
    ...
    'GET /api/tournament-stats': async () => {
      ...
      try {
        matchesData = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'wc2026', 'matches.json'), 'utf8'));
      } catch { return { error: 'matches.json not found' }; }
      try {
        squadsData = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'wc2026', 'squads.json'), 'utf8'));
      } catch { squadsData = {}; }
      ...
      const topScorers = Object.entries(scorerMap)
        .map(([key, goals]) => {
          ...
          return { name, goals, team, flag: CODE_TO_FLAG[team] || '🏳️' };
        })
