...
function getMatchBridge() {
  ...
  try { _matchBridge = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'sources', 'seed', 'wc2026', 'match_id_bridge.json'), 'utf8')); } catch {}
  return _matchBridge;
}
function getWeatherData() {
  if (_weatherData) return _weatherData;
  try { _weatherData = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'wc2026', 'weather.json'), 'utf8')); } catch {}
  return _weatherData;
}
