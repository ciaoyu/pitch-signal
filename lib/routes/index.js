...
module.exports = function registerRoutes(routes, deps) {
  ...
  // Coach routes
  const coachRoutes = safeLoadRoutes('coach', require('./coach'), deps);

  // Lineups routes
  const lineupsRoutes = safeLoadRoutes('lineups', require('./lineups'), deps);

  // Ask routes
  const askRoutes = safeLoadRoutes('ask', require('./ask'), deps);

  // Recent routes (recent-matches + recent-stats)
  const recentRoutes = safeLoadRoutes('recent', require('./recent'), deps);
  ...
  // 合并所有成功加载的路由
  Object.assign(routes, coreRoutes, predictionRoutes, entityRoutes, newsRoutes, botRoutes, healthRoutes, matchupRoutes, oddsRoutes, standingsRoutes, venueRoutes, coachRoutes, lineupsRoutes, askRoutes, recentRoutes);
