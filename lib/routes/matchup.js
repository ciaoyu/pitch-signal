module.exports = function createMatchupRoutes(deps) {
  ...
  'GET /api/match/:id/bench': async (params) => {
    ...
    } catch { console.debug('matchup: failed to parse scoreboard competitors (first pass)'); }
    ...
    if (!homeId || !awayId) {
      ...
      } catch { console.debug('matchup: failed to parse summary competitors (fallback)'); }
    }
  ...
  'GET /api/matchup/:id/formation': async (params) => {
    ...
    } catch { console.debug('matchup: failed to parse scoreboard competitors (second route)'); }
    ...
    if (!homeId || !awayId) {
      ...
      } catch { console.debug('matchup: failed to parse summary competitors (second fallback)'); }
    }
    ...
    try {
      ...
      if (lineupsData?.homeXI && lineupsData?.awayXI) {
        ...
        const buildRatings = (xi) => {
          const r = {};
          for (const p of xi) {
            const pid = p.fifaPlayerId || p.name;
            r[pid] = { name: p.name, pos: p.pos === 'GK' ? 'GK' : FP_TO_POS[p.fieldPos] || 'CM', rating: 70 };
          }
          return r;
        };
        const homeTeamName = getTeamNameZh(homeId) || homeId;
        ...
        ratingsWrapped.data[homeTeam.id] = { formation: lineupsData.homeFormation || '4-3-3', players: buildRatings(lineupsData.homeXI) };
        ratingsWrapped.data[awayTeam.id] = { formation: lineupsData.awayFormation || '4-3-3', players: buildRatings(lineupsData.awayXI) };
        const result = matchupAPI.handleMatchupFormation(
          {}, params.id, homeTeam, awayTeam, ratingsWrapped, getCached(`odds_${params.id}`) || {}, null
        );
        return result;
      }
    } catch (e) { console.debug('matchup: FIFA lineups lookup failed:', e.message); }
      ...
      if (rosters.length >= 2) {
        ...
        const buildFromEspnRoster = (rosterEntry, teamId) => {
          ...
          const players = starters.map((p, i) => {
            ...
            return {
              id: p.athlete?.id || `p${i}`,
              name: p.athlete?.shortName || p.athlete?.displayName || `#${p.jersey}`,
        ...
        if (homeBuilt.players.length >= 11 && awayBuilt.players.length >= 11) {
          ...
          return {
            home: { team: homeName, teamId: homeId, formation: homeBuilt.formation, ...groupByLine(homeBuilt.players) },
            away: { team: awayName, teamId: awayId, formation: awayBuilt.formation, ...groupByLine(awayBuilt.players) },
            summary: { homeAdvantages: 0, awayAdvantages: 0, evenPairs: 0, pairs: [] },
          };
        }
      }
    } catch (e) { console.debug('matchup: ESPN roster path failed:', e.message); }
      ...
      try {
        ...
        if (rosters.length >= 2) {
          ...
          try {
            ...
            return result;
          } catch { console.debug('matchup: matchupAPI.handleMatchupFormation failed'); }
        }
      } catch { console.warn('matchup: both scoreboard and summary competitor parsing failed'); }
  ...
  'GET /api/corner-analysis/:id': async (params) => {
    ...
    } catch { console.warn('matchup: ESPN summary fetch failed'); }
    ...
    } catch { console.debug('matchup: failed to parse competitors from summary (third route)'); }
    ...
    } catch { console.debug('corner-analysis: recent-stats fallback to defaults'); }
  ...
  'GET /api/matchup-spatial/:home/:away': async (params) => {
    ...
    try {
      // 1. 将 ESPN team ID 转换为 FIFA 代码（idBridge 由 server.js 加载并注入 deps）
      ...
      if (homeFifaCode && awayFifaCode) {
        // 2. 查找比赛 ID
        const matchesPath = path.join(DATA_DIR, 'wc2026', 'matches.json');
        ...
        if (match) {
          ...
          if (lineupsData.hasRealLineups) {
            matchOptions = {
              ...
              substitutions: lineupsData.substitutions,
              source: 'official',
      ...
      console.warn('[matchup-spatial] 获取阵容数据失败:', e.message);
    }
