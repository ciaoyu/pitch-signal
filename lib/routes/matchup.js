...
module.exports = function createMatchupRoutes(deps) {
  ...
  const { estimateMatchCorners, getCornerReferenceLine } = require('../corner-model');
  const { parseAddedTime } = require('../live-helpers');
  ...
  'GET /api/corner-analysis/:id': async (params) => {
    ...
    let homeId = '', awayId = '';
    try {
      for (const c of (matchData?.header?.competitions?.[0]?.competitors || [])) {
        if (c.homeAway === 'home') homeId = String(c.id || c.team?.id || '');
        else if (c.homeAway === 'away') awayId = String(c.id || c.team?.id || '');
      }
    } catch { logger.debug('matchup: failed to parse competitors from summary (corner route)'); }
    try {
      if (!homeId || !awayId) {
        const sb = await espn('/scoreboard', 'scores', 60000);
        for (const ev of (sb?.events || [])) {
    ...
    // Tournament corner averages. cornersAgainst is derived from the opponent's
    // wonCorners in the same ESPN boxscore, so both sides use the same sample.
    let homeAvgCorners = null;
    let awayAvgCorners = null;
    let homeAvgAgainst = null;
    let awayAvgAgainst = null;
    let homeSampleCount = 0;
    let awaySampleCount = 0;
    try {
      ...
      if (recentStatsRoute && homeId && awayId) {
        ...
        if (homeRecent?.stats?.wonCorners?.avg >= 0) homeAvgCorners = homeRecent.stats.wonCorners.avg;
        if (awayRecent?.stats?.wonCorners?.avg >= 0) awayAvgCorners = awayRecent.stats.wonCorners.avg;
        if (homeRecent?.stats?.cornersAgainst?.avg >= 0) homeAvgAgainst = homeRecent.stats.cornersAgainst.avg;
        if (awayRecent?.stats?.cornersAgainst?.avg >= 0) awayAvgAgainst = awayRecent.stats.cornersAgainst.avg;
        homeSampleCount = homeRecent?.stats?.wonCorners?.count || 0;
        awaySampleCount = awayRecent?.stats?.wonCorners?.count || 0;
      }
    } catch { logger.debug('corner-analysis: recent-stats unavailable; use tournament prior'); }

    const cornerEstimate = estimateMatchCorners({
      homeFor: homeAvgCorners,
      awayFor: awayAvgCorners,
      homeAgainst: homeAvgAgainst,
      awayAgainst: awayAvgAgainst,
      homeForCount: homeSampleCount,
      awayForCount: awaySampleCount,
      homeAgainstCount: homeSampleCount,
      awayAgainstCount: awaySampleCount,
    });
    const homePredicted = cornerEstimate.home;
    const awayPredicted = cornerEstimate.away;
    const totalPredicted = cornerEstimate.total;

    // A reference line is a manually supplied comparison point, not bookmaker
    // odds. The current Odds API request only fetches h2h and cannot supply a
    // corner market.
    const oddsData = getCached(`odds_${params.id}`) || {};
    const marketCorners = oddsData.corners || {};
    const hasVerifiedMarket = marketCorners.kind === 'market'
      && Number.isFinite(Number(marketCorners.line))
      && (marketCorners.over != null || marketCorners.under != null);
    const referenceLine = getCornerReferenceLine(params.id);
    const lineInfo = hasVerifiedMarket
      ? { ...marketCorners, line: Number(marketCorners.line) }
      : referenceLine;
    const oddsLine = lineInfo?.line ?? null;
    const hasComparisonLine = Number.isFinite(oddsLine);
    ...
    if (matchData) {
      ...
      minute = (parseInt(displayClock, 10) || 0) + parseAddedTime(displayClock);
    }
    ...
    // Over/Under verdict
    const diff = hasComparisonLine ? realtimePredicted - oddsLine : null;
    let trend, confidence;
    if (!hasComparisonLine || Math.abs(diff) < 0.5) { trend = 'neutral'; confidence = 'low'; }
    else if (diff > 1.5) { trend = 'over_strong'; confidence = 'medium'; }
    else if (diff > 0.5) { trend = 'over_slight'; confidence = 'medium'; }
    else if (diff < -1.5) { trend = 'under_strong'; confidence = 'medium'; }
    else { trend = 'under_slight'; confidence = 'medium'; }

    // Pace
    const paceBaseline = hasComparisonLine ? oddsLine : totalPredicted;
    const expectedByNow = (minute / 90) * paceBaseline;
    let pace = 'on_track';
    if (totalCurrent < expectedByNow - 0.5) pace = 'below';
    else if (totalCurrent > expectedByNow + 0.5) pace = 'above';

    // Progress bar
    const progress = paceBaseline > 0 ? Math.round((totalCurrent / paceBaseline) * 100) : 0;
    ...
    const fallbackReasonI18n = hasComparisonLine
      ? { zh: '预测与参考线接近，暂无明确倾向', en: 'Projection is close to the reference line; no clear lean' }
      : { zh: '没有可信比较线，暂不判断大小方向', en: 'No trustworthy comparison line; no over/under lean' };
    ...
    return {
      ...
      historical: {
        ...
        homeAgainstAvg: homeAvgAgainst,
        awayAgainstAvg: awayAvgAgainst,
        homeSampleCount,
        awaySampleCount,
      },
      predicted: {
        ...
        priorTotal: cornerEstimate.priorTotal,
        sampleWeight: cornerEstimate.sampleWeight,
        method: cornerEstimate.method,
        status: cornerEstimate.status,
      },
      odds: {
        line: oddsLine,
        over: hasVerifiedMarket ? marketCorners.over : null,
        under: hasVerifiedMarket ? marketCorners.under : null,
        kind: lineInfo?.kind || 'unavailable',
        source: lineInfo?.source || null,
        asOf: lineInfo?.asOf || null,
      },
      realtime: {
        ...
        vsOddsLine: minute > 0 && hasComparisonLine ? Math.round(((totalCurrent / minute) * 90 - oddsLine) * 10) / 10 : null,
      },
      progress: {
        actual: progress,
