...
// 博彩/赌球来源过滤——产品定位为"非投注建议"，不展示博彩内容。
// exclude_domains 给 Tavily 做源头过滤；looksLikeBetting 兜底拦标题/域名明显是博彩的漏网项。
...
function generateMockNews(homeTeam, awayTeam, matchDate, homeI18n = null, awayI18n = null, matchStatus = null) {
  ...
  const hoursUntilMatch = Number.isFinite(matchTime.getTime())
    ...
    : 0;
  const isFinished = matchStatus === 'post' || matchStatus === 'STATUS_FINAL' || (matchTime.getTime() > 0 && now > matchTime && hoursUntilMatch < -2);
...
module.exports = function createNewsRoutes(deps) {
  ...
  return {
    'GET /api/match/:id/news': async (params) => {
      ...
      const isFinished = ctx.matchStatus === 'post' || ctx.matchStatus === 'STATUS_FINAL';

      const queries = isFinished
        ? [
            `${ctx.homeTeam} ${ctx.awayTeam} result highlights`,
            `${ctx.homeTeam} ${ctx.awayTeam} match analysis`,
            `${ctx.homeTeam} post-match reaction`,
            `${ctx.awayTeam} post-match reaction`,
            `${ctx.homeTeam} ${ctx.awayTeam} recap`,
          ]
        : [
            `${ctx.homeTeam} vs ${ctx.awayTeam} preview`,
            `${ctx.homeTeam} team news`,
            `${ctx.awayTeam} team news`,
            `${ctx.homeTeam} injury update`,
            `${ctx.awayTeam} injury update`,
          ];
    ...
    'GET /api/match/:id/head-to-head': async (params) => {
      ...
      try {
        ...
        console.warn('[H2H] CSV lookup failed in news.js:', e.message);
      }
