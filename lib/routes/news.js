...
const TRUSTED_NEWS_DOMAINS = [
  'fifa.com', 'espn.com', 'apnews.com', 'reuters.com', 'bbc.com', 'bbc.co.uk',
  'theathletic.com', 'lemonde.fr', 'lequipe.fr', 'elpais.com', 'as.com',
  'marca.com', 'aljazeera.com', 'theanalyst.com', 'foxsports.com', 'uefa.com',
  'fff.fr', 'rfef.es',
];

function isTrustedNewsUrl(url) {
...
function looksLikeBetting(title, content, url) {
  ...
  return /betting tip|bet of the day|accumulator|best odds|best bets?|\bodds?\b|sportsbook|betting preview|gambling|free bet|acca\b/.test(text);
}
...
function extractEspnNews(summary, homeTeam, awayTeam) {
  const home = String(homeTeam || '').toLowerCase();
  const away = String(awayTeam || '').toLowerCase();
  return (summary?.news?.articles || [])
    .filter(article => {
      const title = article.headline || '';
      const description = article.description || '';
      const url = article.links?.web?.href || '';
      const text = `${title} ${description}`.toLowerCase();
      return url && isTrustedNewsUrl(url) && ((home && text.includes(home)) || (away && text.includes(away)));
    })
    .slice(0, 6)
    .map(article => {
      const title = article.headline || '';
      ...
      const url = article.links?.web?.href || '';
      return {
        id: `espn_${article.id || makeId('news', title)}`,
        title,
        summary: content,
        content,
        url,
  ...
  try {
    ...
    if (homeComp?.team && awayComp?.team) {
      ...
      return {
        ...
        espnNews: extractEspnNews(summary, homeTeam, awayTeam),
      };
    }
  } catch {}
...
function buildContextAwareSearchTerms(ctx, opts = {}) {
  ...
  const base = isFinished
    ? [
        `${homeTeam} ${awayTeam} World Cup 2026 result highlights`,
        `${homeTeam} ${awayTeam} World Cup 2026 match analysis`,
        `site:fifa.com ${homeTeam} ${awayTeam} World Cup 2026`,
        `${homeTeam} post-match reaction`,
        `${awayTeam} post-match reaction`,
      ]
    : [
        `${homeTeam} vs ${awayTeam} World Cup 2026 preview`,
        `${homeTeam} ${awayTeam} World Cup 2026 team news`,
        `site:fifa.com ${homeTeam} ${awayTeam} World Cup 2026`,
...
module.exports = function createNewsRoutes(deps) {
  ...
  return {
    'GET /api/match/:id/news': async (params) => {
      ...
      let newsItems = [...(ctx.espnNews || [])];
      let source = newsItems.length > 0 ? 'espn' : 'empty';
      let emptyReason = TAVILY_API_KEY ? 'no_results' : 'missing_tavily_key';
      const seen = new Set(newsItems.map(item => item.url).filter(Boolean));
      let tavilyCount = 0;
        ...
        for (const query of queries) {
          try {
            ...
            for (const result of (tavilyData.results || [])) {
              ...
              if (!isTrustedNewsUrl(url)) continue;
      ...
      if (newsItems.length > 0) {
        source = ctx.espnNews?.length > 0
          ? (tavilyCount > 0 ? 'espn+tavily' : 'espn')
          : 'tavily';
        // Translate sourced news (adds titleI18n / summaryI18n / contentI18n / sourceI18n)
        ...
        // Fail closed: an empty news panel is preferable to presenting generated
        // headlines, injuries, or coach quotes as if they were reported facts.
        newsItems = [];
        source = 'empty';
      }
        ...
        try {
          ...
          if (response.ok) {
            ...
            for (const result of (tavilyData.results || [])) {
              const title = result.title || '';
              const content = result.content || '';
              if (!isTrustedNewsUrl(result.url)) continue;
