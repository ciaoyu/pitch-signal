  ...
  return {
    ...
    'GET /api/team/:id/recent-stats': async (params) => {
      ...
      try {
        ...
        const targetMatchIds = new Set(pastMatches.map(m => String(m.matchId)));

        // Collect boxscore stats for each match
        const espnByMatchId = {};
        await Promise.all(espnDates.map(async (dateKey) => {
          try {
            const data = await espn(`/scoreboard?dates=${dateKey}`, `s_${dateKey}`, 600000);
            const relevantEvents = (data.events || []).filter(ev => targetMatchIds.has(String(ev.id)));
        ...
        for (const m of pastMatches) {
          ...
          for (const s of entry.teamStats) {
            ...
            if (s.name === 'wonCorners') {
              const opponentRaw = s[opponentSide] || '0';
              const opponentMatch = String(opponentRaw).match(/(\d+(?:\.\d+)?)/);
              statRow.cornersAgainst = opponentMatch ? Number(opponentMatch[1]) : 0;
            }
          }
