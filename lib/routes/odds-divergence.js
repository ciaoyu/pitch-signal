 ...
 * P2-3: 模型 vs 市场赔率分歧
 *
 * 目的：仅作 benchmark，不把赔率喂进模型。
 * 分歧阈值：|model_prob − market_prob| > 0.08 标 divergence。
 *
 * 导出：
  ...
  return {
    ...
    'GET /api/odds-divergence/:matchId': async (params) => {
      ...
      try {
        ...
        const pred = await ps.predictMatch(matchId, { includeExternalOdds: true, bypassCache: true });
        if (pred?.error) return { matchId, error: 'Prediction unavailable', detail: pred.error };
        ...
        // 市场概率：优先从 externalOdds 取十进制赔率
        let marketProbs = null;
        let oddsSource = 'unknown';

        if (pred.externalOdds) {
          const eo = pred.externalOdds;
