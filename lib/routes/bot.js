...
module.exports = function createBotRoutes(deps) {
  ...
  async function fetchMatchContext(matchId, { getCached, setCache }) {
    ...
    try {
      if (predRes.knockoutIntel && Object.keys(predRes.knockoutIntel.sections || {}).length > 0) {
        ctx.knockoutIntel = predRes.knockoutIntel;
      } else {
        const stage = String(predRes.stage || ctx.match?.stage || '').toUpperCase();
        const isKo = predRes.isKnockout || ['R32', 'R16', 'QF', 'SF', 'F', '3RD', 'KO'].includes(stage);
        if (isKo) {
          const { buildKnockoutIntel } = require('../services/knockout-intel');
          const intel = buildKnockoutIntel({
            matchId,
            homeName: ctx.match?.home,
            awayName: ctx.match?.away,
            stage,
          });
          if (intel && Object.keys(intel.sections || {}).length > 0) {
            ctx.knockoutIntel = intel;
          }
        }
      }
    } catch (_) {}
  ...
  async function handleChat(params, body, req) {
    ...
    // P2-6: Global match data (always injected, 30min cache)
