  ...
  async function fetchMatchContext(matchId, { getCached, setCache }) {
    ...
    try {
      ...
      if (!predRes.error) {
        ...
        // KO-1: surface the knockout-intel object (suspensions + future sections)
        // into the bot context so it can be injected verbatim into the prompt.
        if (predRes.knockoutIntel) ctx.knockoutIntel = predRes.knockoutIntel;
      }
    } catch (_) {}
