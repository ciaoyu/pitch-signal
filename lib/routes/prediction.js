  ...
  return {
    ...
    "GET /api/predict/:matchId": async (params) => {
      try {
        ...
        return await predictionService.predictMatch(params.matchId, { includeExternalOdds: hasOddsKey, persist: false });
      } catch (e) {
     ...
     *
     * The server determines isKnockout from the match stage (authoritative), not from the
     * client query parameter. The isKnockout query param is accepted for backwards compat
     * but is overridden by the server-side stage check.
     *
     * Query: ?minute=&homeScore=&awayScore=&homeRed=&awayRed=&addedTime=&hasPenalties=
     * Returns: { regulation, advance, isKnockout, preMatch, current, curve, liveState, score }
     */
    'GET /api/match/:id/live-probability': async (params) => {
      try {
        ...
        // Server-authoritative isKnockout: read from schedule snapshot (not DB,
        // because the matches table has no espn_id column).
        // NEVER trust the client-provided isKnockout boolean. If the schedule
        // is unavailable, treat as group stage and mark the source.
        let stage = '';
        let isKnockout = false;
        let stageSource = 'schedule';
        try {
          const { getKnockoutContextForMatch } = require('../services/PredictionService');
          const koCtx = getKnockoutContextForMatch(matchId);
          stage = koCtx.stage || '';
          isKnockout = koCtx.isKnockout;
        } catch (_) {
          // Schedule snapshot unavailable — treat as group stage.
          // Do NOT fall back to client-provided isKnockout parameter.
          stageSource = 'unavailable';
          isKnockout = false;
        }
        const hasPenalties = params.hasPenalties === 'true';
        ...
        return {
          ...
          // Canonical C-refactor fields
          regulation: liveProb.regulation,
          advance: liveProb.advance,
          isKnockout,
          stageSource,
          // Deprecated flat fields for transition
          current: {
            homeWin: liveProb.regulation.homeWin,
            draw:    liveProb.regulation.draw,
            awayWin: liveProb.regulation.awayWin,
            homeWinAfterET:  liveProb.advance?.homeWinAfterET ?? 0,
            awayWinAfterET:  liveProb.advance?.awayWinAfterET ?? 0,
            penaltyHomeWin:  liveProb.advance?.penaltyHomeWin ?? 0,
            penaltyAwayWin:  liveProb.advance?.penaltyAwayWin ?? 0,
          },
          curve,
          source: 'live_reprice',
        };
