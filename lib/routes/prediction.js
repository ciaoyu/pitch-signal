  ...
  return {
    ...
    'GET /api/match/:id/live-probability': async (params) => {
      try {
        ...
        // Server-authoritative isKnockout: check from match metadata (stage field)
        const matchRow = db.prepare(`
          SELECT stage FROM matches WHERE espn_id = ? OR home_team_id = ? OR away_team_id = ? LIMIT 1
        `).get(matchId, matchId, matchId);
        const stage = matchRow?.stage || params.stage || '';
        const isKnockout = stage ? !/group/i.test(stage) : (params.isKnockout === 'true');
