/**
 * Qualification Situation Analysis - Monte Carlo Simulation
 * Simulates remaining matches, calculates qualification probability for each team
 * v2 - Load real group tables from DB + run simulation driven by real Elo
 */
...
class QualificationSimulator {
  ...
  /**
   * Load real group tables from database
   */
  loadGroupsFromDB() {
  ...
  /**
   * Monte Carlo simulation for group qualification
   * @param {object} group - { name, teams, matches }
   * @returns {object} Qualification probability for each team
   */
  simulateGroup(group) {
  ...
  /**
   * Simultaneously simulate all groups, cross-comparing 12 third-placed teams across groups to select top 8 in each simulation run.
   * Top two statistics per group remain independent; best third-placed teams returned via separate fields.
   */
  simulateGroups(groups) {
    ...
    for (let sim = 0; sim < this.simulations; sim++) {
      ...
      thirdPlaced.sort((a, b) => this.compareStandings(a, b));
  ...
  simulateGroupTable(group) {
    ...
    const simTable = group.teams.map(t => ({
      ...
      conduct: t.conduct,
      teamConductScore: t.teamConductScore,
      fifaRank: t.fifaRank,
    ...
    simTable.sort((a, b) => this.compareStandings(a, b));
    return simTable;
  }
  ...
  compareStandings(a, b) {
    ...
    const aConduct = this.getOptionalNumber(a.teamConductScore, a.conduct);
    const bConduct = this.getOptionalNumber(b.teamConductScore, b.conduct);
    if (aConduct !== null && bConduct !== null && bConduct !== aConduct) {
      return bConduct - aConduct;
    }

    const aFifaRank = this.getOptionalNumber(a.fifaRank, a.fifaRanking);
    const bFifaRank = this.getOptionalNumber(b.fifaRank, b.fifaRanking);
    if (aFifaRank !== null && bFifaRank !== null && aFifaRank !== bFifaRank) {
      return aFifaRank - bFifaRank;
    }

    // Partial/missing FIFA ranking data cannot be compared on the official scale.
    // Use Elo for both teams, then a stable team ID so input group order never decides.
    const aRating = Number(this.ratings[a.id]?.rating) || 1500;
    const bRating = Number(this.ratings[b.id]?.rating) || 1500;
  ...
  /**
   * Simulate match using actual Elo rating strengths
   */
  simulateMatch(match, home, away) {
    if (this.predictionEngine) {
      ...
      // Use expected goals as Poisson parameter
      return {
    ...
    // Fallback: simple simulation based on Elo difference
    const homeR = this.getTeamRating(match.homeId);
    const awayR = this.getTeamRating(match.awayId);
    const eloDiff = (homeR.rating || 1500) - (awayR.rating || 1500);
    
    // Add 0.4 expected goals per 100 points of Elo difference
  ...
  /**
   * Build group data from ratings.json (backward compatibility preservation)
   */
  buildGroupsFromRatings(ratings, standings) {
    // If DB has data, prefer loading from DB
    try {
