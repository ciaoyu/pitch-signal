...
function _repriceCalc(params) {
  ...
  // Fix W1-B (§1.3): pre-match λ (preLambdaHome/Away) is calibrated for a regular
  // 90-minute match. With long added time, totalMinutes > 90 so minutesRemaining/90
  // can exceed 1.0 at kickoff and inflate the remaining λ above the pre-match
  // baseline. Cap the ratio at 1.0 so the live λ never exceeds the pre-match expectation.
