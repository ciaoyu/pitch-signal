...
### 8.0 两人分工与 worktree 规约

**按"文件领地"切成两条线，最小化合并冲突：**

| | 开发者 A —— 赛中与引擎线 | 开发者 B —— 评估与校准线 |
|---|---|---|
| **领地文件** | `lib/live-reprice.js`、`lib/routes/`、`lib/services/PredictionService.js`、`lib/elo.js`、`static/` 前端 | `lib/backtest.js`、校准/基准工具、数据管线、`scripts/` |
| **Wave 1** | W1-A、W1-B | W1-C、W1-D |
| **Wave 2** | W2-E、W2-G、W2-I | W2-F、W2-H |
| **Wave 3** | W3-J | W3-K、W3-L |

切分理由：W2-E 与 W2-G 都改 `lib/elo.js`，必须同人（A）；W2-F/W2-H 依赖 W1-C/W1-D 自建的工具，B 自建自用；W2-I 赢者接入 `live-reprice.js`（A 领地），但其第一步数据可得性审计用 B 的工具——这是唯一协作点，A 超载时审计步骤可由 B 在 Wave 1 尾声代做。

**Worktree / 分支命名（一任务一 worktree，位于 `.claude/worktrees/<名>`，分支同名前缀 `feat/`）：**

| 任务 | 分支 | worktree 目录名 | 负责人 |
|---|---|---|---|
| W1-A | `feat/w1a-live-track-discipline` | `w1a-live-track-discipline` | A |
| W1-B | `feat/w1b-deterministic-bugs` | `w1b-deterministic-bugs` | A |
| W1-C | `feat/w1c-eval-baselines` | `w1c-eval-baselines` | B |
| W1-D | `feat/w1d-data-pipeline-audit` | `w1d-data-pipeline-audit` | B |
| W2-E | `feat/w2e-draw-model` | `w2e-draw-model` | A |
| W2-F | `feat/w2f-calibration` | `w2f-calibration` | B |
| W2-G | `feat/w2g-neutral-hfa` | `w2g-neutral-hfa` | A |
| W2-H | `feat/w2h-ablation-decay` | `w2h-ablation-decay` | B |
| W2-I | `feat/w2i-inplay-comparison` | `w2i-inplay-comparison` | A（审计步骤可由 B 代做） |
| W3-J | `feat/w3j-market-tiers` | `w3j-market-tiers` | A |
| W3-K | `feat/w3k-quantile-intervals` | `w3k-quantile-intervals` | B |
| W3-L | `feat/w3l-lessons-loop` | `w3l-lessons-loop` | B |

**Worktree 纪律：**

1. **基点**：所有任务 worktree 一律从**本路线图已合入后的 `main`** 新建；Wave 2 的 worktree 必须等 Wave 1 全部验收合入 `main` 后再拉，**禁止从彼此的任务分支拉取或跨 worktree 依赖**。
2. **生命周期**：任务验收合入后立即删除该 worktree 与分支；不留长命分支。
3. **越界改动**：确需改对方领地文件时，先通知总控制协调排序，不许静默改。
4. **共同红线**：Wave 1 期间回测主数字（964 场 57.88% / Brier 0.5708）逐位不变；动手前先在自己 worktree 跑一次 `node scripts/run-backtest.js` 留基线记录，PR 附前后对照。
5. **测试互斥**：集成测试会起临时服务器（端口 5099），两个 worktree **不要同时跑 `npm test`**，错开执行；也不许同时对同一 `data/` 起 dev server（SQLite 单写者）。
6. **合并流**：任务分支 → 总控制验收（§8 验收流程）→ merge 进 `main`（push/merge 每次单独经用户确认）→ 删 worktree。
