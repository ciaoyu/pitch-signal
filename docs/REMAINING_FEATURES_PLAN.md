...
| P2-6 Bot 知识库注入出线概率 + 赛事情境 | 🟡 **进行中**（用户当前工作） | `lib/routes/bot.js` 已加 `fetchGlobalContext()` 注入出线概率/积分榜，**尚未提交**；`scripts/test-bot-kb.js` 已写但**未注册进 `scripts/test-runner.js` 的 `TEST_FILES`**。收尾清单：① 跑通并提交 ② 把 test-bot-kb.js 加进 TEST_FILES ③ 手工问几个"谁能出线"类问题验证 Bot 真的引用了注入的数据，不是幻觉 |
