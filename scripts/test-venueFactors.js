...
console.log('📊 2. β_temp 高温衰减');
{
  ...
  // T=50°F → max(0,18)=18 → β=1-0.18=0.82
  r = vf.temperatureFactorF(50);
  assertApprox(r.beta, 0.82, 'T=50°F → β_temp=0.82');
  assert(r.applied === true, 'T=50°F applied=true');

  // T=90°F → max(0,58)=58 → β=1-0.58=0.42
  r = vf.temperatureFactorF(90);
  assertApprox(r.beta, 0.42, 'T=90°F → β_temp=0.42');

  // 摄氏度入口：0°C = 32°F → β=1
  r = vf.temperatureFactor(0);
  assertApprox(r.beta, 1.0, 'T=0°C → β_temp=1');

  // 30°C = 86°F → max(0,54)=54 → β=0.46
  r = vf.temperatureFactor(30);
  assertApprox(r.beta, 0.46, 'T=30°C → β_temp=0.46');

  // 10°C = 50°F → β=0.82
  r = vf.temperatureFactor(10);
  assertApprox(r.beta, 0.82, 'T=10°C → β_temp=0.82');
...
console.log('📊 5b. CPV 兜底（不在 id_bridge，经 teams.json 解析，不抛错）');
{
  ...
  assert(r.source.includes('no_altitude_effect') || r.source.includes('temp:'), 'CPV source 正确');
...
console.log('📊 6. 真实比赛 computeForTeam（baseCamp.altitude 未落盘）');
{
  ...
  // β_temp: 26.9°C = 80.42°F → max(0,48.42)=48.42 → β=1-0.4842=0.5158
  ...
  assert(r.applied === true, 'β_temp 生效 → applied=true');
  assertApprox(r.tempC, 26.9, 'tempC 取自 weather.live');
  assert(r.tempSource === 'weather.live', 'tempSource = weather.live');
  // β = 1 · 0.5158 ≈ 0.5158
  const expectedBeta = Math.round((1 - 0.01 * (26.9 * 9 / 5 + 32 - 32)) * 10000) / 10000;
  assertApprox(r.beta, expectedBeta, 'β = β_alt·β_temp');
  assert(r.source.includes('temp:heat_decay'), 'source 含 temp:heat_decay');
...
console.log('📊 8. 模拟 altitude 落盘后 β_alt 生效（不依赖真实数据）');
{
  ...
  // 组合：β_alt=0.9637, β_temp(30°C=86°F)=0.46 → β=0.4433
  const t = vf.temperatureFactor(30);
  const combined = Math.round(r.beta * t.beta * 10000) / 10000;
  assertApprox(combined, 0.4433, '组合 β = 0.9637 · 0.46');
}
